			...
			try {
				...
				sh '''
					...
					if (env.GERRIT_REFSPEC) {
						...
						checkout([$class: 'GitSCM', branches: [[name: "$GERRIT_PATCHSET_REVISION"]], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: ‘hyperledger-jobbuilder’, name: "", refspec: "$GERRIT_REFSPEC", url: 'git://cloud.hyperledger.org/mirror/fabric-ca.git']]])
					} else {
						git url: 'git://cloud.hyperledger.org/mirror/fabric-ca.git', credentialsId: ‘hyperledger-jobbuilder’
