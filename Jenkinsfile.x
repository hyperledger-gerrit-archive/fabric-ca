			...
			try {
				...
				sh '''
					...
					if(env.GERRIT_REFSPEC && env.GERRIT_PATCHSET_REVISION) {
						...
						checkout([$class: 'GitSCM', branches: [[name: "$GERRIT_PATCHSET_REVISION"]], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: ‘hyperledger-jobbuilder’, name: "", refspec: "$GERRIT_REFSPEC", url: 'git://cloud.hyperledger.org/mirror/fabric.git']]])
					} else {
						git url: 'git://cloud.hyperledger.org', credentialsId: ‘hyperledger-jobbuilder’
