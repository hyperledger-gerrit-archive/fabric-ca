...
import (
	...
	cadbutil "github.com/hyperledger/fabric-ca/lib/server/userregistry/db/util"
...
// NewAccessor is a constructor for the database API
func NewAccessor(db db.FabricCADB) *Accessor {
	return &Accessor{
		db: db,
	}
}
...
func (d *Accessor) DeleteUser(id string) (userregistry.User, error) {
...
func (d *Accessor) GetUser(id string, attrs []string) (userregistry.User, error) {
...
func (d *Accessor) DeleteAffiliation(name string, force, identityRemoval, isRegistrar bool) (*userregistry.DbTxResult, error) {
	...
	deletedInfo := result.(*userregistry.DbTxResult)
...
func (d *Accessor) GetAffiliationTree(name string) (*userregistry.DbTxResult, error) {
	...
	getResult := result.(*userregistry.DbTxResult)
...
func (d *Accessor) GetUserLessThanLevel(level int) ([]userregistry.User, error) {
	if level == 0 {
		return []userregistry.User{}, nil
	}
...
func (d *Accessor) ModifyAffiliation(oldAffiliation, newAffiliation string, force, isRegistrar bool) (*userregistry.DbTxResult, error) {
	...
	modifiedInfo := result.(*userregistry.DbTxResult)
		...
		if len(idsWithOldAff) > 0 {
			...
			if force {
				...
				for _, userRec := range idsWithOldAff {
					...
					userAff := cadbuser.GetUserAffiliation(user)                          // Get the current affiliation
...
func (d *Accessor) getResult(ids []cadbuser.Record, affs []db.AffiliationRecord) *userregistry.DbTxResult {
	// Collect all the identities that were modified
	identities := []userregistry.User{}
	for _, id := range ids {
	...
	return &userregistry.DbTxResult{
