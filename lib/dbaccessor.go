		...
		if len(idsWithOldAff) > 0 {
			...
			if force {
				...
				for _, userRec := range idsWithOldAff {
					user := d.newDBUser(&userRec)
					currentAttrs, _ := user.GetAttributes(nil)
					// Get all current user attributes
...
func checksDupAttrNames(attrs []api.Attribute) error {
	for _, attr := range attrs {
		...
		for _, attr2 := range attrs {
			...
			if found > 1 {
				return newHTTPErr(403, ErrDuplicateAttrReq, "Attribute '%s' is present multiple times; can't request multiple attributes with same name", attr.Name)
			}
		}
	}
	return nil
