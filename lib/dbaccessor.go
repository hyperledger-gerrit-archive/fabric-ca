...
const (
	entryNotFoundError        = "%s not found in the database"
	replaceEntryNotFoundError = "sql: no rows in result set"
)

func dbGetError(err error, prefix string) error {
	if err.Error() == replaceEntryNotFoundError {
		return fmt.Errorf(entryNotFoundError, prefix)
	}
	return err
}
