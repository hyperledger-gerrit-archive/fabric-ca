...
func (d *Accessor) ModifyIdentity(id, update, newConfig string) error {
	...
	var supportedModifyReq = map[string]string{
		"secret":         "token",
		"type":           "type",
	...
	modifyReq, validRequest := supportedModifyReq[update]
	if !validRequest {
	...
	transact := func() error {
		// If modifying secret, generate a hash for the secret
		if modifyReq == "token" {
			pwd := []byte(newConfig)
			pwd, err = bcrypt.GenerateFromPassword(pwd, bcrypt.DefaultCost)
			if err != nil {
		...
		if modifyReq == "attributes" {
			...
			var currentAttributes []api.Attribute
			json.Unmarshal([]byte(userRec.Attributes), &currentAttributes)
			var newAttr api.Attribute
			for _, newAttr = range newAttributes {
				foundAttr := false
				for i := range currentAttributes {
					if currentAttributes[i].Name == newAttr.Name {
						currentAttributes[i].Value = newAttr.Value
						foundAttr = true
						break
					}
				}
				if !foundAttr {
					currentAttributes = append(currentAttributes, newAttr)
				}
			}

			attrBytes, err := json.Marshal(currentAttributes)
			if err != nil {
				return err
			}
		...
		query := fmt.Sprintf("UPDATE users SET %s = ? where (id = ?)", modifyReq)
...
// GetRole returns the type of the user
func (u *DBUser) GetRole() string {
	return u.Type
}
