...
func (d *Accessor) GetFilteredUsers(affiliation, types string) ([]spi.User, error) {
	...
	if affiliation == "" {
		...
		err = readUserRows(rows, &subAffIds)
		if err != nil {
			return nil, err
		}
	}
...
func readUserRows(rows *sqlx.Rows, ids *[]UserRecord) error {
	for rows.Next() {
		var id UserRecord
		err := rows.StructScan(&id)
		if err != nil {
			return errors.Wrap(err, "Failed to read the row")
		}
		*ids = append(*ids, id)
	}
	return nil
