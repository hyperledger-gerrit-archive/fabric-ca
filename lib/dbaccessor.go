...
func (d *Accessor) GetFilteredUsers(affiliation, types string) (*sqlx.Rows, error) {
	...
	if affiliation == "" {
		if strings.Contains(types, "*") { // If type is '*', allowed to get back of all types
	...
	if strings.Contains(types, "*") { // If type is '*', allowed to get back of all types for requested affiliation
