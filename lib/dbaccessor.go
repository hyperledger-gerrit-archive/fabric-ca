...
func (d *Accessor) deleteAffiliationTx(tx *sqlx.Tx, args ...interface{}) (interface{}, error) {
	...
	_, err = tx.Exec(tx.Rebind("DELETE FROM affiliations where (name = ? OR name LIKE ?)"), name, subAffName)
	if err != nil {
...
func (d *Accessor) modifyAffiliationTx(tx *sqlx.Tx, args ...interface{}) (interface{}, error) {
	...
	query := "SELECT name, prekey FROM affiliations WHERE (name = ? OR name LIKE ?)"
	var allOldAffiliations []AffiliationRecord
	err := tx.Select(&allOldAffiliations, tx.Rebind(query), oldAffiliation, oldAffiliation+".%")
	if err != nil {
		return nil, err
	}
