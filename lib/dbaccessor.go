...
func (d *Accessor) DeleteUser(id string) (*spi.DbTxResult, error) {
	...
	_, err := d.doTransaction(deleteUserTx, id, 6) // 6 (cessationofoperation) reason for certificate revocation
	if err != nil {
		return nil, err
	}
...
func deleteUserTx(tx *sqlx.Tx, args ...interface{}) (interface{}, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "Error encountered while revoking certificates for deleted identities")
	}
...
func (d *Accessor) deleteAffiliationTx(tx *sqlx.Tx, args ...interface{}) (interface{}, error) {
	...
	if len(ids) > 0 {
		...
		if force {
			log.Debugf("IDs '%s' to removed based on affiliation '%s' removal", idNames, name)
			...
			// Revoke all the certificates associated with the removed identities above with reason of "affiliationchange" (4)
			query = "UPDATE certificates SET status='revoked', revoked_at=CURRENT_TIMESTAMP, reason = 4 WHERE (id IN (?) AND status != 'revoked')"
			inQuery, args, err = sqlx.In(query, idNames)
			if err != nil {
				return nil, newHTTPErr(500, ErrRemoveAffDB, "Failed to construct query '%s': %s", query, err)
			}
