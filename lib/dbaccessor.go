...
func (d *Accessor) DeleteUser(id string) (spi.User, error) {
	...
	result, err := d.doTransaction(d.deleteUserTx, id)
	if err != nil {
		return nil, err
	}
...
func (d *Accessor) deleteUserTx(tx *sqlx.Tx, args ...interface{}) (interface{}, error) {
	...
	if err != nil {
		return nil, newHTTPErr(500, ErrDBDeleteUser, "Error deleting certificates for '%s': %s", id, err)
	}
...
func (d *Accessor) getResult(ids []UserRecord, affs []spi.AffiliationImpl) *spi.DbTxResult {
