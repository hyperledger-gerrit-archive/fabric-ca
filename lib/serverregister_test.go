...
func TestRegistrarAttribute(t *testing.T) {
	...
	if assert.Error(t, err, "Should have failed to register an identity with inappropriate attributes") {
		assert.Contains(t, err.Error(), "Registrar does not have any values for 'hf.Registrar.Attributes' thus can't register any attributes")
	}
	...
	assert.Error(t, err, "Should have failed to register an identity with an attribute that is not part of 'hf.Registrar.Attributes'")
	...
	if assert.Error(t, err, "Should have failed to register an identity with inappropriate attributes") {
		assert.Contains(t, err.Error(), "cannot be a pattern")
	}
	...
	if assert.Error(t, err, "Should have failed to register an identity with inappropriate attributes") {
		assert.Contains(t, err.Error(), "is not part of")
	}
