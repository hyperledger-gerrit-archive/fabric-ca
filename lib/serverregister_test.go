...
func TestRegistrarAttribute(t *testing.T) {
	...
	id = CAConfigIdentity{
		...
		Attrs: map[string]string{
			...
			attrRegistrarAttr:  "a.b.c, x.y.*, testattr*, attr$, hf.Registrar.Attributes, hf.Revoker",
		},
	}
	registry.Identities = append(registry.Identities, id)
...
func invalidAttrRequestValues(t *testing.T, registrar *Identity) {
	...
	if assert.Errorf(t, err, "Should have failed to register an identity with inappropriate values for '%s', can only register a subset", attrRoles) {
...
func TestFixedValueAttributes(t *testing.T) {
	os.RemoveAll(rootDir)
	os.RemoveAll("../testdata/msp")
	defer os.RemoveAll(rootDir)
	defer os.RemoveAll("../testdata/msp")
	...
	adminID := enrollResp.Identity

	// Test registration and enrollment of an identity with attributes
	userName := "badHfEnrollmentID"
	registerReq := &api.RegistrationRequest{
		Name:        userName,
		Type:        "client",
		Affiliation: "hyperledger",
		Attributes: []api.Attribute{
			api.Attribute{Name: "hf.EnrollmentID", Value: "invalidID", ECert: true},
		},
	}
	_, err = adminID.Register(registerReq)
	assert.Error(t, err, "Can't register attribute 'hf.EnrollmentID' with a value that does not equal the enrollment ID")
	...
	registerReq = &api.RegistrationRequest{
		...
		Affiliation: "hyperledger",
		Attributes: []api.Attribute{
			api.Attribute{Name: "hf.Type", Value: "peer", ECert: true},
		},
	}
	_, err = adminID.Register(registerReq)
	assert.Error(t, err, "Can't register attribute 'hf.Type' with a value that does not equal the enrollment ID")

	// Test registration and enrollment of an identity with attributes
	userName = "badHfAffiliation"
	registerReq = &api.RegistrationRequest{
		Name:        userName,
		Type:        "client",
		Affiliation: "hyperledger",
		Attributes: []api.Attribute{
			api.Attribute{Name: "hf.Affiliation", Value: "org1", ECert: true},
		},
	}
	_, err = adminID.Register(registerReq)
	assert.Error(t, err, "Can't register attribute 'hf.Affiliation' with a value that does not equal the enrollment ID")
}
