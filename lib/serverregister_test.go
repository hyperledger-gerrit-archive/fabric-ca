...
func TestRegistrarAttribute(t *testing.T) {
	...
	id = CAConfigIdentity{
		...
		Attrs: map[string]string{
			...
			attrRegistrarAttr:  "a.b.c, x.y.*, testattr*, attr$",
		},
	}
	registry.Identities = append(registry.Identities, id)
	...
	// Negative case: Registrar does not own this attribute nor does it have the attribute 'hf.Registrar.Attribute'
	...
	if assert.Error(t, err, "Should have failed to register an identity with inappropriate attributes") {
		assert.Contains(t, err.Error(), "Registrar does not own 'fake.attribute' attribute")
	}

	// Negative case: Registrar does not own 'hf.Registrar.Attribute'
	_, err = registrar.Identity.Register(registerTestUser("user1",
		[]api.Attribute{
			api.Attribute{
	...
	if assert.Error(t, err, "Should have failed to register an identity with inappropriate attributes") {
		assert.Contains(t, err.Error(), "Registrar cannot register")
	}

	// Positive case: Registrar owns this attribute but does not have the attribute 'hf.Registrar.Attribute'
	_, err = registrar.Identity.Register(registerTestUser("user1",
		[]api.Attribute{
			api.Attribute{
				Name:  attrRevoker,
	...
	if assert.Error(t, err, "Should have failed to register an identity with inappropriate attributes") {
		assert.Contains(t, err.Error(), "Registrar does not own")
	}
	...
	// Positive case: Registrar's hf.Registrar.Attribute = a.b.c, x.y.*, testattr*
	_, err = registrar.Identity.Register(registerTestUser("user3_1",
		[]api.Attribute{
			api.Attribute{
				Name:  "testattr1",
				Value: "val1",
	...
	if assert.Error(t, err, "Should have failed to register an identity with inappropriate attributes") {
		assert.Contains(t, err.Error(), "Registrar is not allowed to register attribute")
	}
	...
	// Positive case: Registrar's hf.Registrar.Attribute = a.b.c, x.y.*, testattr*, attr$
	_, err = registrar.Identity.Register(registerTestUser("user7_1",
		[]api.Attribute{
			api.Attribute{
				Name:  "attr$",
				Value: "val1",
