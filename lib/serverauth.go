...
func (ah *fcaAuthHandler) serveHTTP(w http.ResponseWriter, r *http.Request) error {
	...
	caName := r.Header.Get("caname")
	if caName == "" {
		caName = DefaultCAName
	}

	// Look up CA to see if CA exist by that name
	if _, ok := ah.server.CAs[caName]; !ok {
		return fmt.Errorf("No CA exist by the name '%s'", caName)
	}
	...
	case basic:
		...
		if ok {
			...
			u, err := ah.server.CAs[caName].registry.GetUser(user, nil)
			if err != nil {
	...
	case token:
		...
		// verify token
		cert, err2 := util.VerifyToken(ah.server.CAs[caName].csp, authHdr, body)
		if err2 != nil {
		...
		certs, err := ah.server.CAs[caName].CertDBAccessor().GetCertificate(serial, aki)
		if err != nil {
			return authError
		}
