...
func (ah *fcaAuthHandler) crlListSizeCheck(cert *x509.Certificate) error {
	...
	for _, url := range cert.CRLDistributionPoints {
		...
		crlSize, _ := strconv.Atoi(r.Header.Get("Content-Length"))

		log.Debugf("Size of CRL list is %d bytes", crlSize)

		if crlSize >= crlSizeLimit {
			return fmt.Errorf("The size of the requested CRL list size is %d, which is greater than the CRL size limit of %d", crlSize, crlSizeLimit)
		}
	}
