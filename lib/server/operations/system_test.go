...
var _ = Describe("System", func() {
	...
	BeforeEach(func() {
		...
		config = operations.Config{
			...
			Metrics: operations.MetricsConfig{
				Provider: "disabled",
			},
			TLS: operations.TLS{
				Enabled:            true,
		...
		system = operations.NewSystem(&config)
	...
	Context("when ClientCertRequired is true", func() {
		BeforeEach(func() {
			config.TLS.ClientCertRequired = true
			system = operations.NewSystem(&config)
		})
	...
	Context("when a bad TLS configuration is provided", func() {
		BeforeEach(func() {
			config.TLS.CertFile = "cert-file-does-not-exist"
			system = operations.NewSystem(&config)
		})
	...
	Context("when the metrics provider is disabled", func() {
		BeforeEach(func() {
			config.Metrics = operations.MetricsConfig{
				Provider: "disabled",
			}
			system = operations.NewSystem(&config)
	...
	Context("when the metrics provider is statsd", func() {
		...
		Context("when checking the network and address fails", func() {
			BeforeEach(func() {
				config.Metrics.Statsd.Network = "bob-the-network"
				system = operations.NewSystem(&config)
			})
	...
	Context("when the metrics provider is unknown", func() {
		BeforeEach(func() {
			config.Metrics.Provider = "something-unknown"
			system = operations.NewSystem(&config)
		})
