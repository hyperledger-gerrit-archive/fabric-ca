...
func TestStoreInvalidRevocationPublicKeyFile(t *testing.T) {
	revocationKey, err := idemix.GenerateLongTermRevocationKey()
	if err != nil {
		t.Fatalf("Failed to generate ECDSA key for revocation authority")
	}
	pubkeyfile := "./testdata1/RevocationPublicKey"
	idemixLib := new(mocks.Lib)
	rk := NewRevocationKey(pubkeyfile, testRevocationPrivateKeyFile, idemixLib)
