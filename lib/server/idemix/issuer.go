...
func (i *issuer) VerifyToken(req *http.Request, body []byte) (string, error) {
	if !i.isInitialized {
	...
	msg := req.Method + "." + b64uri + "." + util.B64Encode(body)
	digest, digestError := i.csp.Hash([]byte(msg), &bccsp.SHAOpts{})
	if digestError != nil {
		return "", errors.WithMessage(digestError, fmt.Sprintf("Failed to create authorization token '%s'", msg))
	}
