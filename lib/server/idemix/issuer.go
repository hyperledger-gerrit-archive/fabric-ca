...
type Issuer interface {
	...
	VerifyToken(tokenVersion int, req *http.Request, body []byte) (string, error)
}
...
func (i *issuer) VerifyToken(tokenVersion int, req *http.Request, body []byte) (string, error) {
	if !i.isInitialized {
	...
	if tokenVersion == 2 {
		...
		payload = req.Method + "." + b64uri + "." + util.B64Encode(body)
	} else {
		payload = util.B64Encode(body)
	}
	digest, digestError := i.csp.Hash([]byte(payload), &bccsp.SHAOpts{})
	if digestError != nil {
		return "", errors.WithMessage(digestError, fmt.Sprintf("Failed to create authentication token '%s'", payload))
	}
	...
	sigBytes, err := util.B64Decode(parts[2])
	if err != nil {
