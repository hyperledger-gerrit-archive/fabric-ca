...
func TestNewIssuer(t *testing.T) {
	...
	issuer := NewIssuer("ca1", ".", cfg, db, lib)
...
func TestInit(t *testing.T) {
	...
	issuer := getIssuer(t, testdir, false, false)
	assert.NotNil(t, issuer)
	err = issuer.Init(false, &dbutil.Levels{Credential: 1, RAInfo: 1, Nonce: 1})
...
func TestInitDBNotInitialized(t *testing.T) {
	...
	issuer := NewIssuer("ca1", ".", cfg, db, NewLib())
	err := issuer.Init(false, &dbutil.Levels{Credential: 1, RAInfo: 1, Nonce: 1})
	assert.NoError(t, err)
...
func TestInitRenewTrue(t *testing.T) {
	...
	defer os.RemoveAll(testdir)
	issuer := getIssuer(t, testdir, true, false)
	assert.NotNil(t, issuer)

	err = issuer.Init(true, &dbutil.Levels{Credential: 1, RAInfo: 1, Nonce: 1})
	assert.Error(t, err, "Init should fail if it fails to generate random number")

	issuer = getIssuer(t, testdir, false, true)
	assert.NotNil(t, issuer)
	err = issuer.Init(true, &dbutil.Levels{Credential: 1, RAInfo: 1, Nonce: 1})
...
func getIssuer(t *testing.T, testDir string, getranderror, newIssuerKeyerror bool) Issuer {
	err := os.MkdirAll(filepath.Join(testDir, "msp/keystore"), 0777)
	if err != nil {
		t.Fatalf("Failed to create directory: %s", err.Error())
	}
	...
	issuer := NewIssuer("ca1", testDir, cfg, db, lib)
	return issuer
}
