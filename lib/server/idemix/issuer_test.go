...
func TestVerifyTokenError(t *testing.T) {
	...
	req.Header = map[string][]string{
		"Authorization": {"idemix.1.foo.blah"},
	}
	_, err = issuer.VerifyToken(req, body)
	...
	req.Header = map[string][]string{
		"Authorization": {"idemix.1.foo.sig"},
	}
	_, err = issuer.VerifyToken(req, body)
...
func TestVerifyTokenBadSignature(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "http://localhost:7054/enroll", reader)
	req.Header = map[string][]string{
		"Authorization": {"idemix.1.admin." + sig},
	}
	_, err = issuer.VerifyToken(req, body)
