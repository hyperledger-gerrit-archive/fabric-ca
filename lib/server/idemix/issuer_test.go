...
func TestVerifyTokenError(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "http://localhost:7054/enroll", reader)
	req.Header = map[string][]string{
		"Authorization": {"idemix.foo.blah"},
	}
	_, err = issuer.VerifyToken(2, req, body)
	...
	req.Header = map[string][]string{
		"Authorization": {"idemix.foo"},
	}
	_, err = issuer.VerifyToken(2, req, body)
	assert.Error(t, err, "VerifyToken should fail if the auth header does not have three parts separated by '.'")
