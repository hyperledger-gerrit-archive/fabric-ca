...
const (
	...
	InsertRCInfo = "INSERT into revocation_authority_info(epoch, private_key, public_key, next_handle, lasthandle_in_pool, level) VALUES (:epoch, :private_key, :public_key, :next_handle, :lasthandle_in_pool, :level)"
...
func (ra *revocationAuthority) getRCInfoFromDB() (*RevocationAuthorityInfo, error) {
	rcinfos := []RevocationAuthorityInfo{}
	err := ra.db.Select(&rcinfos, SelectRCInfo)
	if err != nil {
		return nil, err
	}
	if len(rcinfos) == 0 {
...
func (ra *revocationAuthority) addRCInfoToDB(rcInfo *RevocationAuthorityInfo) error {
	res, err := ra.db.NamedExec(InsertRCInfo, rcInfo)
	if err != nil {
