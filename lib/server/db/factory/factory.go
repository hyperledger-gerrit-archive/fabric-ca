...
func New(dbType, datasource, caName string, tlsConfig *tls.ClientTLSConfig, csp bccsp.BCCSP) (DB, error) {
	switch dbType {
	case "sqlite3":
		return sqlite.NewDB(datasource, caName), nil
	case "postgres":
		return postgres.NewDB(datasource, caName, tlsConfig), nil
	case "mysql":
		return mysql.NewDB(datasource, caName, tlsConfig, csp), nil
	default:
