...
func TestNew(t *testing.T) {
	...
	gt.Expect(db).To(Equal(sqlite.NewDB("fabric_ca.db", "")))

	db, err = factory.New("postgres", "fabric_ca_postgres", "", nil, nil)
	gt.Expect(err).NotTo(HaveOccurred())
	gt.Expect(db).NotTo(BeNil())
	gt.Expect(db).To(Equal(postgres.NewDB("fabric_ca_postgres", "", nil)))

	db, err = factory.New("mysql", "fabric_ca_mysql", "", nil, nil)
	gt.Expect(err).NotTo(HaveOccurred())
	gt.Expect(db).NotTo(BeNil())
	gt.Expect(db).To(Equal(mysql.NewDB("fabric_ca_mysql", "", nil, nil)))
