...
// NewUserRegistry creates a SQLite based user registry
func NewUserRegistry(datasource string) *Sqlite {
...
func (s *Sqlite) Connect() error {
	...
	s.SqlxDB, err = sqlx.Connect("sqlite3", s.datasource+"?_busy_timeout=5000")
	if err != nil {
...
func (s *Sqlite) Create() (*sqlx.DB, error) {
	s.CreateTx = s.SqlxDB.MustBegin()
	err := s.CreateTables()
	if err != nil {
		return nil, err
	}
	return s.SqlxDB.(*sqlx.DB), nil
}
...
func createSQLiteIdentityTable(tx Create) error {
...
func createSQLiteAffiliationTable(tx Create) error {
...
func createSQLiteCertificateTable(tx Create) error {
...
func createSQLiteCredentialsTable(tx Create) error {
...
func createSQLiteRevocationComponentTable(tx Create) error {
...
func createSQLiteNoncesTable(tx Create) error {
