...
var _ = Describe("Postgres", func() {
	...
	BeforeEach(func() {
		...
		db = postgres.NewUserRegistry("host=localhost port=5432 user=root password=rootpw dbname=fabric_ca", "", tls, &disabled.Provider{})
		mockDB = &mocks.FabricCADB{}
	})

	Context("open connection to database", func() {
		It("fails to connect if the contains incorrect syntax", func() {
			db = postgres.NewUserRegistry("host=localhost port=5432 user=root password=rootpw dbname=fabric-ca", "", nil, &disabled.Provider{})
			err := db.Connect()
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).Should(Equal("Database name 'fabric-ca' cannot contain any '-' or end with '.db'"))

			db = postgres.NewUserRegistry("host=localhost port=5432 user=root password=rootpw dbname=fabric_ca.db", "", nil, &disabled.Provider{})
			err = db.Connect()
			Expect(err).To(HaveOccurred())
	...
	Context("pinging database", func() {
		It("returns an error if unable to ping database", func() {
			mockDB.PingReturns(errors.New("ping error"))
			db.SqlxDB = mockDB
