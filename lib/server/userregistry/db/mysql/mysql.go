...
// NewUserRegistry create a MySQL based user registry
func NewUserRegistry(datasource string, clientTLSConfig *tls.ClientTLSConfig, csp bccsp.BCCSP) *Mysql {
...
func (m *Mysql) Create() (*sqlx.DB, error) {
...
func (m *Mysql) CreateDatabase() (*sqlx.DB, error) {
	datasource := m.datasource
	dbName := m.dbName
...
func (m *Mysql) createDatabase() error {
	...
	_, err := m.SqlxDB.Exec("CREATE DATABASE IF NOT EXISTS " + dbName)
	if err != nil {
