...
// BCCSP Signer
func LoadX509KeyPair(certFile string, csp bccsp.BCCSP) (tls.Certificate, error) {
	...
	if err != nil {
		return fail(fmt.Errorf("Could not get the private key that matches %s: %s", certFile, err))
	}
