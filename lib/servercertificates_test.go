...
func TestServerGetCertificates(t *testing.T) {
	...
	attr, err := util.Marshal([]api.Attribute{api.Attribute{Name: "hf.Registrar.Roles", Value: "client"}}, "attributes")
	util.FatalError(t, err, "Failed to marshal attributes")
	user := &dbutil.UserRecord{
		Name:       "Registrar",
		Attributes: string(attr),
	...
	mockCtx.On("HasRole", "hf.Revoker").Return(nil)
	...
	mockCtx.On("GetCaller").Return(dbutil.NewDBUser(user, nil), nil)
