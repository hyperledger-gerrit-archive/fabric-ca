...
func handleEnroll(ctx *serverRequestContext, id string) (interface{}, error) {
	...
	err = csrChecks(id, &req.SignRequest, ca)
	if err != nil {
		return nil, err
	}
...
func csrChecks(id string, req *signer.SignRequest, ca *CA) error {
...
func csrInputLengthCheck(req *x509.CertificateRequest) error {
	log.Debug("Checking that input do not exceed maximum limits")
	...
	for _, n := range req.Subject.Names {
		...
		case n.Type.Equal(countryOID):
			fmt.Println("country value: ", value)
