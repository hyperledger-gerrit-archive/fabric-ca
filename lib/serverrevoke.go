...
func revokeHandler(ctx *serverRequestContext) (interface{}, error) {
	...
	if req.Serial != "" && req.AKI != "" {
		...
		if err != nil {
			return nil, newErr(ErrRevCertNotFound, 404, "Certificate with serial %s and AKI %s was not found: %s",
				req.Serial, req.AKI, err)
		}

		if req.Name != "" && req.Name != certificate.ID {
			return nil, newErr(ErrCertNotOwnedBy, 400, "Certificate with serial %s and AKI %s is not owned by %s",
				req.Serial, req.AKI, req.Name)
		}

		userInfo, err := registry.GetUserInfo(certificate.ID)
		if err != nil {
			return nil, newErr(ErrRevokeIDNotFound, 404, "Identity %s was not found: %s", certificate.ID, err)
		}
		...
		if err != nil {
			return nil, newErr(ErrRevokeFailure, 500, "Revoke of certificate <%s,%s> failed: %s", req.Serial, req.AKI, err)
		}
	} else if req.Name != "" {

		user, err := registry.GetUser(req.Name, nil)
		if err != nil {
			return nil, newErr(ErrRevokeIDNotFound, 404, "Identity %s was not found: %s", req.Name, err)
		}
		...
		if user != nil {
			...
			if err != nil {
				return nil, newErr(ErrRevokeUserInfoNotFound, 500, "Failed getting info for identity %s: %s", req.Name, err)
			}
			...
			if err != nil {
				return nil, newErr(ErrRevokeUpdateUser, 500, "Failed to update identity info: %s", err)
			}
		}
		...
		if err != nil {
			return nil, newErr(ErrNoCertsRevoked, 500, "Failed to revoke certificates for '%s': %s",
				req.Name, err)
		}
		...
		return nil, newErr(ErrMissingRevokeArgs, 400, "Either Name or Serial and AKI are required for a revoke request")
	}
...
func checkAffiliations(revoker string, revoking spi.UserInfo, ca *CA) error {
	...
	if err != nil {
		return newErr(ErrGettingAffiliation, 500, "Failed to get affiliation of %s: %s", revoker, err)
	}
	...
	for i := range revokerAffiliation {
		if revokerAffiliation[i] != revokingAffiliation[i] {
			return newErr(ErrRevokerNotAffiliated, 401,
