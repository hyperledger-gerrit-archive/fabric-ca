...
func testMasqueradeEnroll(t *testing.T, c *Client, id *Identity) {
	...
	_, err = masqueradeEnroll(c, "admin", &api.EnrollmentRequest{
		Name:   name,
		Secret: rr.Secret,
	})
	if err == nil {
		t.Fatalf("%s masquerading as admin should have failed", name)
	}
	log.Debugf("testMasqueradeEnroll error: %s", err)
}
...
func masqueradeEnroll(c *Client, id string, req *api.EnrollmentRequest) (*EnrollmentResponse, error) {
	err := c.Init()
	if err != nil {
		return nil, err
	}
