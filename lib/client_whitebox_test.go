...
func testImpersonation(id *Identity, t *testing.T) {
	...
	/*
		   fakeCert, err := x509.ParseCertificate(fakeCertBytes)
			if err != nil {
				t.Fatalf("Failed to parse self-signed fake cert: %s", err)
			}
		   fakeCert.SerialNumber = cert.SerialNumber
		   fakeCert.AuthorityKeyId = cert.AuthorityKeyId
		   buf, err := asn1.Marshal(fakeCert)
	*/
