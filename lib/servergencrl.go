...
func genCRLHandler(ctx *serverRequestContext) (interface{}, error) {
	...
	resp := &api.GenCRLResponse{CRL: crl}
	return resp, nil
}

// GenCRL will generate CRL
func genCRL(ca *CA, req api.GenCRLRequest) (string, error) {
	var err error
	...
	if req.RevokedAfter.After(before) {
		...
		return "", errors.New("invalid revokedafter value. It must not be a timestamp greater than revokedbefore")
	}
	...
	if err != nil {
		...
		return "", errors.WithMessage(err, "failed to get revoked certificates from the database")
	}
	if len(certs) == 0 {
		return "", errors.Errorf("no revoked certificates found between %s and %s",
			req.RevokedAfter.Format(time.RFC3339), before.Format(time.RFC3339))
	}
	...
	if err != nil {
		...
		return "", errors.Wrapf(err, "failed to get certficate for the CA %s", ca.HomeDir)
	}
	...
	if err != nil {
		...
		return "", errors.WithMessage(err, fmt.Sprintf("failed to get signer for the CA %s", ca.HomeDir))
	}

	var revokedCerts []pkix.RevokedCertificate
	if req.Expiry.IsZero() {
		// add three months expiration time for the CRL if expiry is not set
		req.Expiry = time.Now().UTC().AddDate(0, 3, 0)
	}
