...
func genCRL(ca *CA, req api.GenCRLRequest) ([]byte, error) {
	...
	// certExpWindow, err := time.ParseDuration(ca.Config.CRL.CertExpirationWindow)
	// if err != nil {
	// 	log.Errorf("Failed to parse the 'crl.certexpirationwindow' configuration property '%s', it must be in a format expected by time.ParseDuration. Defaulting to 1 hour",
	// 		ca.Config.CRL.CertExpirationWindow)
	// 	certExpWindow = time.Hour
	// }
	curTime := time.Now().UTC().Add(-time.Second * ca.Config.CRL.CertExpirationWindow)
	...
	// expDur, err := time.ParseDuration(ca.Config.CRL.Expiry)
	// if err != nil {
	// 	log.Errorf("Failed to parse the 'crl.expiry' configuration property '%s', it must be in a format expected by time.ParseDuration. Defaulting to 24 hours",
	// 		ca.Config.CRL.Expiry)
	// 	expDur = time.Hour * 24
	// }
