...
func newTCertHandler(server *Server, caName string) (h http.Handler, err error) {
	handler := &cfsslapi.HTTPHandler{
		Handler: &tcertHandler{server: server, caName: caName},
		Methods: []string{"POST"},
	}
...
func (h *tcertHandler) initTCertHandler(server *Server) (err error) {
	log.Debug("Initializing TCert handler")
	keyfile := server.CAs[DefaultCAName].Config.CA.Keyfile
	certfile := server.CAs[DefaultCAName].Config.CA.Certfile
	...
	rootKey, err := genRootKey(server.CAs[h.caName].csp)
	if err != nil {
		return err
	}
	h.keyTree = tcert.NewKeyTree(server.CAs[h.caName].csp, rootKey)
...
func (h *tcertHandler) handle(w http.ResponseWriter, r *http.Request) error {
	...
	err := h.initTCertHandler(h.server)
	if err != nil {
...
func (h *tcertHandler) getUserInfo(id string, attrNames []string) ([]tcert.Attribute, []string, error) {
	user, err := h.server.CAs[h.caName].registry.GetUser(id, attrNames)
	if err != nil {
		return nil, nil, err
	}
	if err != nil {
