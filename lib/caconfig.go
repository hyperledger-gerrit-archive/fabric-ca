...
// CRLConfig contains configuration options used the gencrl request handler
type CRLConfig struct {
	// Specifies expiration (in days) for the CRL generated by the gencrl request
	// The number of days  specified by this property is added to the UTC time, resulting time
	// is used to set the 'Next Update' date of the CRL
	Expiry int `def:"1" help:"Expiration (in days) for the CRL generated by the gencrl request"`
	// When searching for unexpired revoked certificates, this value is subtracted from the current
	// time to allow for clock skews across systems. If the subtracted current time is less than
	// the expiry value of a revoked certificate, the certificate is considered as unexpired.
	CertExpirationWindow int `def:"10" help:"This value in seconds is used to adjust current time to account for clock skews when querying for unexpired revoked certificates to include in the CRL generated by the gencrl request"`
}
