...
type CRLConfig struct {
	...
	// time to allow for clock skews across systems. If the adjusted current time is less than
	// the expiry value of a revoked certificate, the certificate is considered as unexpired.
	CertExpirationWindow time.Duration `def:"1h" help:"Current time adjuster when generating a CRL without expired certificates"`
}
