...
func TestIdemixCredRevokedUser(t *testing.T) {
	...
	assert.NoError(t, err, "Failed to register 'user1' by 'admin' user")
	...
	assert.NoError(t, err, "Failed to enroll 'user1'")
	...
	assert.NoError(t, err, "Failed to revoke 'user1' by 'admin' user")
