...
const (
	...
	// TLSKeyCertFileNotExistErr is the error that is logged when TLS key file is specified but it does not exist
	TLSKeyCertFileNotExistErr = "If TLS key file %s is specified, it and corresponding certificate file %s should exist"
)
...
func (s *Server) listenAndServe() (err error) {
	...
	if c.TLS.Enabled {
		...
		if c.TLS.KeyFile != "" {
			if !util.FileExists(c.TLS.KeyFile) || !util.FileExists(c.TLS.CertFile) {
				return fmt.Errorf(TLSKeyCertFileNotExistErr, c.TLS.KeyFile, c.TLS.CertFile)
			}
		} else {
