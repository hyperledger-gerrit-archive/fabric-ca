...
func (s *Server) Stop() error {
	...
	// is not received in 3 seconds, return
	port := s.Config.Port
	for i := 0; i < 3; i++ {
		select {
		case <-s.wait:
			log.Debugf("Stop: received server stopped message from listener on port %d", port)
			close(s.wait)
			s.wait = nil
			return nil
		default:
	...
	log.Debugf("Stop: successful on port %d", port)
	return nil
}
