...
func (s *Server) RegisterBootstrapUser(user, pass, affiliation string) error {
	// Initialize the config, setting defaults, etc
	if user == "" || pass == "" || affiliation == "" {
		return errors.New("empty arguments not allowed")
	}
	err := s.initConfig()
	if err != nil {
...
// loadGroupsTable adds the configured affiliation groups to the table
func (s *Server) loadGroupsTable() error {
	log.Debug("Loading groups table")
	err := s.loadGroupsTableR(s.Config.Affiliations, "")
	if err != nil {
		return err
	}
...
// Recursive function to load the groups table from hierarchy
func (s *Server) loadGroupsTableR(val interface{}, parentPath string) (err error) {
	var path string
	if val == nil {
		return nil
	}
	switch val.(type) {
	case string:
		path = groupPath(val.(string), parentPath)
		err = s.addGroup(path, parentPath)
		if err != nil {
			return err
		}
	case []interface{}:
		for _, item := range val.([]interface{}) {
			err = s.loadGroupsTableR(item, parentPath)
			if err != nil {
				return err
			}
		}
	default:
		for name, ele := range val.(map[string]interface{}) {
			path = groupPath(name, parentPath)
			err = s.addGroup(path, parentPath)
			if err != nil {
				return err
			}
			err = s.loadGroupsTableR(ele, path)
			if err != nil {
				return err
			}
		}
...
func (s *Server) addGroup(path, parentPath string) error {
...
func groupPath(name, parent string) string {
	if parent == "" {
		return name
	}
