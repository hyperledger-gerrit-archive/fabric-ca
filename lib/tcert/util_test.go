...
func TestValidateTCertBatchRequest(t *testing.T) {
	...
	keySigBatch, batchError := GetKeySigBatch(bccsp, batchRequest, "SHA2_256")
	if batchError != nil {
...
func TestSignatureValidation(t *testing.T) {
	...
	var isValid bool
	hashOpts, error := HashNameToOpts("SHA2_256")
	if error != nil {
		t.Errorf("String to HashOpts conversion failed with error : %s", error)
	}
	ecSignature, pubASN1, signError := ECDSASignDirect(bccsp, hashOpts)
	if signError != nil {
	...
	hashOpts, error = HashNameToOpts("SHA2_384")
	if error != nil {
		t.Errorf("String to HashOpts conversion failed with error : %s", error)
	}
	ecSignature, pubASN1, signError = ECDSASignDirect(bccsp, hashOpts)
	...
	hashOpts, error = HashNameToOpts("SHA3_256")
	if error != nil {
		t.Errorf("String to HashOpts conversion failed with error : %s", error)
	}
	ecSignature, pubASN1, signError = ECDSASignDirect(bccsp, hashOpts)
	...
	hashOpts, error = HashNameToOpts("SHA3_384")
	if error != nil {
		t.Errorf("String to HashOpts conversion failed with error : %s", error)
	}
	ecSignature, pubASN1, signError = ECDSASignDirect(bccsp, hashOpts)
...
func TestInvalidSignature(t *testing.T) {
	...
	hashOpts, error := HashNameToOpts("SHA2_256")
	if error != nil {
		t.Errorf("Hash String to Opt conversion failed with error : %s", error)
	}
	ecSignature, pubASN1, signError := ECDSASignDirect(bccsp, hashOpts)
	if signError != nil {
		t.Errorf("ECDSA Signature creation failed with error :%s", signError)
...
func TestPubkeyArrayFromBatchRequest(t *testing.T) {
	...
	pubKeySigBatch, error := GetKeySigBatch(bccsp, getBatch, "SHA2_256")
	if error != nil {
...
func getTCertMgr(t *testing.T) *Mgr {
	//Default instancd of bccsp is used
