...
func (tm *Mgr) VerifyMessage(signature api.KeySig) (bool, error) {
	...
	hasOpt, error := HashNameToOpts(hashAlgo)
	if error != nil {
		return false, fmt.Errorf("HashOpts failed with error : %s", error)
	}

	digest, digestError := tm.BCCSP.Hash(pubkeyRaw, hasOpt)
...
func (tm *Mgr) VerifyTCertBatchRequest(tcertbatch *api.GetTCertBatchRequestNet) (bool, error) {
	...
	if keySigBatchLength == 0 {
		return false, errors.New("None of the public key in batch is signed")
	}
	for i := 0; i < keySigBatchLength; i++ {

		isValid, validationError := tm.VerifyMessage(keySigBatch[i])
		if validationError != nil {
			return false, fmt.Errorf("Signature Validation failed with error :%s", validationError)
		}
		if !isValid {
			return false, errors.New("Signature Validation failed")
		}
...
func GetKeySigBatch(bccsp bccsp.BCCSP, batchRequest *api.GetTCertBatchRequest, hashAlgo string) ([]api.KeySig, error) {

	var tempCrypto api.KeySig
	var keySigBatch []api.KeySig

	count := batchRequest.Count

	hashOpts, error := HashNameToOpts(hashAlgo)
	if error != nil {
		return nil, errors.New("HashNameToOpts call failed")
	}
	for i := 0; i < count; i++ {
		...
		keySigBatch = append(keySigBatch, tempCrypto)

	}

	return keySigBatch, nil
}

// ECDSASignDirect Signs message using ECDSA using BCCSP
// @returns Signature byte , Public Key byte and error
func ECDSASignDirect(currentBCCSP bccsp.BCCSP, hashOpts bccsp.HashOpts) ([]byte, []byte, error) {
	...
	digest, digestError := currentBCCSP.Hash(pubkeyRaw, hashOpts)
	if digestError != nil {
...
// HashNameToOpts converts hash algo string to bccsp hashOpts object
func HashNameToOpts(name string) (bccsp.HashOpts, error) {
	...
	default:
		return nil, fmt.Errorf("Hash algorithm passed [%s] is not supported", name)
	}
}
