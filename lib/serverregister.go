...
func registerUser(req *api.RegistrationRequestNet, registrar string, ca *CA) (string, error) {
	...
	if err != nil {
		return "", errors.Wrapf(err, "Registration of '%s' to validate", req.Name)
	}
...
func registerUserID(req *api.RegistrationRequestNet, ca *CA) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "The delegateRoles field is a superset of roles")
	}
...
func isValidAffiliation(affiliation string, ca *CA) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Failed getting affiliation '%s'", affiliation)
	}
...
func canRegister(registrar string, userType string, ca *CA) error {
	...
	if err != nil {
		return errors.Wrap(err, "Registrar does not exist")
	}
