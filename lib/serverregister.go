...
func validateRequestedAttr(reqAttrs []api.Attribute, registrar spi.User) error {
	...
	if registrarAttrs == "" {
		e := newHTTPErr(401, ErrMissingRegAttr, "Registrar does not have any values for '%s' thus can't register any attributes", attrRegistrarAttr)
		return e
	}
	...
	for _, reqAttr := range reqAttrs {
		...
		if reqAttrName == attrRegistrarAttr {
			...
			if err != nil {
				return newHTTPErr(401, ErrRegAttrAuth, "Registrar is now allowed to register attribute '%s': %s", reqAttrName, err)
			}
