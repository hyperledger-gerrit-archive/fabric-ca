...
func registerUser(req *api.RegistrationRequestNet, registrar string, ca *CA, ctx *serverRequestContext) (string, error) {
	...
	_, registrarAffiliation, err := ctx.GetUserInfo([]string{"hf.Registrar.Roles"})
	if err != nil {
...
func validateRequestedAttr(reqAttrs []api.Attribute, registrar spi.User) error {
	...
	if registrarAttrs == "" {
		e := newHTTPErr(400, ErrRegAttr, "Registrar does not have any values for '%s' thus can't register any attributes", attrRegistrarAttr)
		return e
	}
	...
	registrarCanRegisterAttr := func(requestedAttr string) error {
		...
		return newAuthErr(ErrRegAttr, "Attribute is not part of '%s' attribute", attrRegistrarAttr)
	}

	for _, reqAttr := range reqAttrs {
		reqAttrName := reqAttr.Name // Name of the requested attribute
		if strings.HasSuffix(reqAttrName, "*") {
			return newHTTPErr(400, ErrRegAttr, "Requested attribute '%s' cannot be a pattern", reqAttrName)
		}
		...
		if reqAttrName == attrRegistrarAttr {
			...
			if err != nil {
				return newAuthErr(ErrRegAttr, "Registrar is now allowed to register attribute '%s': $s", attrRegistrarAttr, err)
			}
		...
		if err != nil {
			return newAuthErr(ErrRegAttr, "Requested attribute '%s' is not part of registrar's '%s' attribute: %s", reqAttrName, attrRegistrarAttr, err)
		}
	}
