...
func registerUser(req *api.RegistrationRequestNet, registrarRoles tcert.Attribute, registrarAffiliation []string, registrar string, ca *CA) (string, error) {
	...
	err = canRegister(registrarRoles, registrar, req.Type)
	if err != nil {
...
func canRegister(registrarRoles tcert.Attribute, registrar, userType string) error {
	log.Debugf("canRegister - Check to see if registrar '%s' with registrar roles of '%s' can register user type '%s'", registrar, registrarRoles.Value, userType)
