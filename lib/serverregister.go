...
func registerUser(req *api.RegistrationRequest, registrar string, ca *CA, ctx ServerRequestContext) (string, error) {
	...
	if err != nil {
		return "", caerrors.NewHTTPErr2(err, 500, caerrors.ErrRegisteringIdentity, "Registration of '%s' failed", req.Name)
	}
...
func canRegister(registrar spi.User, req *api.RegistrationRequest, ca *CA, ctx ServerRequestContext) error {
	...
	if err != nil {
		return caerrors.WithMessage(err, "Registration of '%s' failed in affiliation validation", req.Name)
	}
