...
func validateRequestedAttr(reqAttrs []api.Attribute, registrar spi.User) error {
	...
	if registrarAttrs == "" {
		e := newHTTPErr(400, ErrMissingRegAttr, "Registrar does not have any values for '%s' thus can't register any attributes", attrRegistrarAttr)
		return e
	}
	...
	for _, reqAttr := range reqAttrs {
		...
		if strings.HasSuffix(reqAttrName, "*") {
			return newHTTPErr(400, ErrAttrFormat, "Requested attribute '%s' cannot be a pattern", reqAttrName)
		}
		...
		if reqAttrName == attrRegistrarAttr {
			...
			if err != nil {
				return newHTTPErr(400, ErrRegAttrAuth, "Registrar is now allowed to register attribute '%s': $s", reqAttrName, err)
			}
