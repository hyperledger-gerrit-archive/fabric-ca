...
func TestGetCredentialsByIDSelectError(t *testing.T) {
	...
	q := fmt.Sprintf(SelectCredentialByIDSQL, sqlstruct.Columns(IdemixCredRecord{}))
...
func TestGetCredentialSelectError(t *testing.T) {
	...
	cr := IdemixCredRecord{}
	q := fmt.Sprintf(SelectCredentialSQL, sqlstruct.Columns(IdemixCredRecord{}))
...
func getCredRecord() IdemixCredRecord {
	return IdemixCredRecord{
		ID:               "foo",
		CALabel:          "",
		Expiry:           time.Now(),
		Level:            1,
