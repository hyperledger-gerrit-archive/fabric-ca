...
func TestKeyGenerate(t *testing.T) {
	...
	t.Run("384", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"ecdsa", 384}, false) })
	t.Run("521", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"ecdsa", 521}, true) })
	t.Run("521", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"ecdsa", 224}, true) })
	t.Run("512", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"rsa", 512}, true) })
	t.Run("1024", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"rsa", 1024}, true) })
	t.Run("2048", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"rsa", 2048}, false) })
	t.Run("3072", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"rsa", 3072}, false) })
	t.Run("4096", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"rsa", 4096}, false) })
	t.Run("4097", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"rsa", 4097}, true) })
	t.Run("10000", func(t *testing.T) { testKeyGenerate(t, &csr.BasicKeyRequest{"rsa", 10000}, true) })
...
func TestBccspBackedSigner(t *testing.T) {
	...
	signer, works, err := BccspBackedSigner(root, nil, csp)
	if works {
		t.Fatal("BccspBackedSigner should not be valid for empty cert")
	}
	...
	signer, works, err = BccspBackedSigner(root, nil, csp)
	if !works {
		t.Fatal("BccspBackedSigner should had found cert")
	}
