...
func testRevocation(c *Client, t *testing.T, user string, withPriv, ecertOnly bool) {
	...
	if ecertOnly {
		...
		assert.Equal(t, 1, len(allRevResp.RevokedCerts), "Expected 1 certificate to be revoked")
		assert.Equal(t, util.GetSerialAsHex(cert.SerialNumber), allRevResp.RevokedCerts[0].Serial,
			"Cert serial in revocation response does match serial number of the cert that was revoked")
	}
