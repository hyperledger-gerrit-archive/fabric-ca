...
func TestGenCRL(t *testing.T) {
	...
	// Error case 2: revokedafter is greater than revokedbefore
	gencrlReq = &api.GenCRLRequest{
		CAName:        "",
	...
	// Error case 3: expireafter is greater than expirebefore
	gencrlReq = &api.GenCRLRequest{
		CAName:       "",
	...
	// Error case 4:  gencrl request should fail if there are no revoked certs
	gencrlReq = &api.GenCRLRequest{
		CAName:       "",
		ExpireBefore: time.Now().UTC().AddDate(0, 4, 0),
		ExpireAfter:  time.Now().UTC().AddDate(0, 3, 0),
	}
	_, err = adminID.GenCRL(gencrlReq)
	assert.Error(t, err, "genCRL should have failed as there are no revoked certs that expire in the specified period")
	assert.Contains(t, err.Error(), "No revoked certificates found between", "Not expected error message")

	// Error case 5: gencrl request by an user without hf.GenCRL authority should fail
