...
func TestClientInit(t *testing.T) {
	client := new(Client)
	client.Config = new(ClientConfigImpl)
...
func TestIdemixEnroll(t *testing.T) {
	...
	client := &Client{
		Config:  &ClientConfigImpl{URL: fmt.Sprintf("http://localhost:%d", ctport1)},
		HomeDir: clientHome,
	}
...
func testGetCAInfo(c *Client, t *testing.T) {
	...
	client2 := new(Client)
	client2.Config = new(ClientConfigImpl)
...
func testRegister(c *Client, t *testing.T) {
	...
	attrs, err := eresp.Identity.GetX509Credential().Attributes()
	if err != nil {
		t.Fatalf("%s", err)
	}
...
func testRevocation(c *Client, t *testing.T, user string, withPriv, ecertOnly bool) {
	...
	if withPriv {
		...
		cert := id.GetX509Credential().GetX509Cert()
...
func testRevocationErrors(c *Client, t *testing.T) {
	...
	revreq.Name = "fake"
	revreq.Serial, revreq.AKI, err = GetCertID(eresp.Identity.GetX509Credential().Cert())
...
func setupGenCRLWithIntServerTest(t *testing.T, rootCAHome, intCAHome, clientHome string, noCrlSign bool) (*Server, *Server, *Identity) {
	...
	// Enroll admin
	c := &Client{
		Config:  &ClientConfigImpl{URL: fmt.Sprintf("http://localhost:%d", intCAPort)},
		HomeDir: clientHome,
	}
...
func TestRevokedIdentity(t *testing.T) {
	...
	// Bad Init
	c2 := new(Client)
	c2.Config = new(ClientConfigImpl)
