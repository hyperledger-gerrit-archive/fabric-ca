...
func TestTokenVersions(t *testing.T) {
	setupServerClient := func(index, srvPort, authHdrVer int) (*Server, *Client, *Client) {
		...
		client := &Client{
			Config:  &ClientConfig{URL: fmt.Sprintf("http://localhost:%d", srvPort), AuthHdrVersion: authHdrVer},
		...
		badClient := &Client{
			Config:  &ClientConfig{URL: fmt.Sprintf("http://localhost:%d", srvPort), AuthHdrVersion: clientAuthHdrVer},
