...
func (ctx *serverRequestContextImpl) TokenAuthentication() (string, error) {
	...
	if idemix.IsToken(authHdr) {
		return ctx.ca.issuer.VerifyToken(ctx.ca.server.Config.AuthHeaderVer, r, body)
	}
...
func (ctx *serverRequestContextImpl) verifyX509Token(ca *CA, authHdr string, body []byte) (string, error) {
	...
	cert, err2 := util.VerifyToken(ca.csp, ctx.ca.server.Config.AuthHeaderVer, authHdr, ctx.req.Method, ctx.req.URL.RequestURI(), body)
	if err2 != nil {
