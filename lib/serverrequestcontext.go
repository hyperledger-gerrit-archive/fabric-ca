...
func (ctx *serverRequestContext) BasicAuthentication() (string, error) {
	...
	if !ok {
		return "", ctx.authErr(ErrNoUserPass, "No user/pass in authorization header")
	}
...
func (ctx *serverRequestContext) TokenAuthentication() (string, error) {
	...
	if err2 != nil {
		return "", ctx.authErr(ErrInvalidToken, "Invalid token in authorization header: %s", err2)
	}
	// Make sure the caller's cert was issued by this CA
	err2 = ca.VerifyCertificate(cert)
	if err2 != nil {
		return "", ctx.authErr(ErrUntrustedCertificate, "Untrusted certificate: %s", err2)
	}
	...
	if expired {
		return "", ctx.authErr(ErrCertExpired,
	...
	if len(certs) == 0 {
		return "", ctx.authErr(ErrCertNotFound, "Certificate not found with AKI '%s' and serial '%s'", aki, serial)
	}
	for _, certificate := range certs {
		if certificate.Status == "revoked" {
			return "", ctx.authErr(ErrCertRevoked, "The certificate in the authorization header is a revoked certificate")
		}
	}
	ctx.enrollmentID = id
...
func (ctx *serverRequestContext) authErr(code int, format string, args ...interface{}) error {
