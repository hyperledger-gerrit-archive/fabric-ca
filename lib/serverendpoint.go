...
func (se *serverEndpoint) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	url := r.URL.String()
	log.Debugf("TLE: Received request for %s", url)
	...
	if err == nil {
		...
		log.Debugf("TLE: Sent response for %s: %+v", url, resp)
	} else {
		var he *httpErr
		switch err.(type) {
		case *httpErr:
		...
		lcode = he.lcode
		he.writeResponse(w)
		log.Debugf("TLE: Sent error for %s: %+v", url, he)
	}
	// Create access log entry
	log.Infof("ALE: %s - \"%s %s\" %d %d", r.RemoteAddr, r.Method, r.URL, scode, lcode)
}
