	...
	if r.Method == "HEAD" {
		...
		if err != nil {
			...
			if err != nil {
				...
				log.Debugf("Sent response with errors for %s: %+v", url, maskPassword(resp))
			}
		} else {
			...
			if err != nil {
				...
				log.Debugf("Sent response for %s: %+v", url, maskPassword(resp))
			}
		}
		log.Infof(`%s %s %s 200 0 "OK"`, r.RemoteAddr, r.Method, r.URL)
	} else if err != nil {
...
func maskPassword(resp interface{}) string {
	respStr := fmt.Sprintf("%+v", resp)
	re := regexp.MustCompile(`(\bPassword: ([^,}\] ]+))`)
	respStr = re.ReplaceAllString(respStr, `"Password": ****`)
	return respStr
}
