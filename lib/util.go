...
func IdentityDecoder(decoder *json.Decoder, storePath string) error {
...
func AffiliationDecoder(decoder *json.Decoder, storePath string) error {
...
func NewCertificateDecoder() *CertificateDecoder {
...
func (cd *CertificateDecoder) CertificateDecoder(decoder *json.Decoder, storePath string) error {
	var cert certPEM
	...
	if storePath != "" {
		return cd.StoreCert(enrollmentID, storePath, []byte(cert.PEM))
	}
...
func (cd *CertificateDecoder) StoreCert(enrollmentID, storePath string, cert []byte) error {
	...
	case 1: // Only certificate return don't need to append number to certificate file name
		filePath = filepath.Join(storePath, singleCertName)
	case 2: // Two certificate returned, rename old certificate to have number at the end
