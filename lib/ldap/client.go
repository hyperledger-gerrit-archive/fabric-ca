...
func (lc *Client) GetUserInfo(id string) (spi.UserInfo, error) {
	var ui spi.UserInfo
	user, err := lc.GetUser(id, nil)
	if err != nil {
		return ui, err
	}
	ui.Name = user.GetName()
	ui.Affiliation = strings.Join(user.GetAffiliationPath(), ".")
	attrs := user.GetAttributes(lc.attrNames)
	rattrs := make([]api.Attribute, len(attrs))
	for idx, attr := range attrs {
		rattrs[idx].Name = attr.Name
		rattrs[idx].Value = attr.Value
	}
	ui.Attributes = rattrs
	return ui, nil
}
