...
type AttrConfig struct {
	...
	Maps       map[string][]MapResponse
}

// Converter provides a way to convert an LDAP entry to the value of a fabric-ca-server attribute
type Converter struct {
	Name  string // the fabric-ca-server attribute name
	Value string // LDAP expression to evaluate to get the value for this attribute
}

// MapResponse provides a way to map LDAP responses to other values
...
func NewClient(cfg *Config, csp bccsp.BCCSP) (*Client, error) {
	...
	for mapName, value := range cfg.Attribute.Maps {
		...
		for _, ele := range value {
			c.attrMaps[mapName][ele.From] = ele.To
			log.Debugf("Added '%s' -> '%s' to LDAP map '%s'", ele.From, ele.To, mapName)
		}
	}
	c.TLS = &cfg.TLS
	c.CSP = csp
