...
func (i *Identity) RevokeAll(req *api.RevocationRequest) (*api.AllRevocationResponse, error) {
	...
	crl, err := util.B64Decode(result.X509Revocation.CRL)
	if err != nil {
		return nil, err
	}
	result.X509Revocation.CRL = string(crl)
	log.Debugf("Successfully revoked certificates: %+v", req)
	return &api.AllRevocationResponse{X509Revocation: result.X509Revocation, IdemixRevocation: result.IdemixRevocation}, nil
}
