...
func (i *Identity) RegisterAndEnroll(req *api.RegistrationRequest) (*Identity, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Failed to register %s", req.Name)
	}
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Failed to enroll %s", req.Name)
	}
...
func (i *Identity) addTokenAuthHdr(req *http.Request, body []byte) error {
	...
	if err != nil {
		return errors.Wrap(err, "Failed to add token authorization header")
	}
