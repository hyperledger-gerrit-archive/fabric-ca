...
func (i *Identity) GetAllIdentities(caname string) error {
	log.Debugf("Entering identity.GetAllIdentities")
	err := i.GetStreamResponse("identities", caname, "identities")
	if err != nil {
		return err
	}
...
func (i *Identity) GetStreamResponse(endpoint, caname, stream string) error {
	req, err := i.client.newGet(endpoint)
	if err != nil {
		return err
	}
	...
	return i.client.StreamResponse(req, stream)
}
