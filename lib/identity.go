...
func (i *Identity) AddAffiliation(req *api.AddAffiliationRequestNet) (*api.AffiliationResponse, error) {
	log.Debugf("Entering identity.AddAffiliation with request: %+v", req)
	if req.Path == "" {
		return nil, errors.New("No new affiliation path specified")
	}
...
func (i *Identity) ModifyAffiliation(req *api.ModifyAffiliationRequestNet) (*api.AffiliationResponse, error) {
	...
	result := &api.AffiliationResponse{}
	err = i.Put(fmt.Sprintf("affiliations/%s", path), reqBody, result)
	if err != nil {
		return nil, err
	}
...
func (i *Identity) RemoveAffiliation(req *api.RemoveAffiliationRequestNet) (*api.RemoveAffiliationResponse, error) {
	...
	err := i.Delete(fmt.Sprintf("identities/%s", path), req.CAName, result)
	if err != nil {
		return nil, err
	}
