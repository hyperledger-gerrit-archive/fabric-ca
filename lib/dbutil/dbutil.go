...
// FabricCADB is the database interface for storing and querying
// data to the Fabric CA database
...
// FabricCATx is database transaction interface for fabric CA related database
// oprerations that need atomicity of transacations
...
func updateSQLiteSchema(db *DB, serverLevels *Levels) error {
	...
	// if currentLevels.Credential < serverLevels.Credential {
	// 	log.Debug("Upgrade credentials table")
	// 	err := doTransaction(db, updateCredentialsTable, currentLevels.Credential)
	// 	if err != nil {
	// 		return err
	// 	}
	// }
