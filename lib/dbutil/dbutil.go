...
func createSQLiteDBTables(datasource string) error {
	...
	if _, err := db.Exec("CREATE TABLE IF NOT EXISTS users (id VARCHAR(256), token bytea, type VARCHAR(256), affiliation VARCHAR(1024), attributes TEXT, state INTEGER,  max_enrollments INTEGER)"); err != nil {
	...
	if _, err := db.Exec("CREATE TABLE IF NOT EXISTS certificates (id VARCHAR(256), serial_number blob NOT NULL, authority_key_identifier blob NOT NULL, ca_label blob, status blob NOT NULL, reason int, expiry timestamp, revoked_at timestamp, pem blob NOT NULL, PRIMARY KEY(serial_number, authority_key_identifier))"); err != nil {
...
func createMySQLTables(dbName string, db *sqlx.DB) error {
	...
	if _, err := db.Exec("CREATE TABLE IF NOT EXISTS users (id VARCHAR(256) NOT NULL, token blob, type VARCHAR(256), affiliation VARCHAR(1024), attributes TEXT, state INTEGER, max_enrollments INTEGER, PRIMARY KEY (id)) DEFAULT CHARSET=utf8 COLLATE utf8_bin"); err != nil {
		return errors.Wrap(err, "Error creating users table")
	}
...
func updateMySQLSchema(db *sqlx.DB) error {
	...
	_, err = db.Exec("ALTER TABLE users MODIFY id VARCHAR(256), MODIFY type VARCHAR(256), MODIFY affiliation VARCHAR(256)")
	if err != nil {
		return err
	}
	...
	_, err = db.Exec("ALTER TABLE certificates MODIFY id VARCHAR(256)")
	if err != nil {
		return err
	}
...
func updatePostgresSchema(db *sqlx.DB) error {
	...
	_, err = db.Exec("ALTER TABLE users ALTER COLUMN id TYPE VARCHAR(256), ALTER COLUMN type TYPE VARCHAR(256), ALTER COLUMN affiliation TYPE VARCHAR(256)")
	if err != nil {
		return err
	}
	...
	_, err = db.Exec("ALTER TABLE certificates ALTER COLUMN id TYPE VARCHAR(256)")
	if err != nil {
		return err
	}
