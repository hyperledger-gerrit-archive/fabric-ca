...
func createMySQLTables(dbName string, db *sqlx.DB) error {
	...
	if _, err := db.Exec("CREATE TABLE IF NOT EXISTS affiliations (name VARCHAR(1024) NOT NULL, prekey VARCHAR(1024), level INTEGER DEFAULT 0, PRIMARY KEY (name)) ROW_FORMAT=DYNAMIC"); err != nil {
		return errors.Wrap(err, "Error creating affiliations table")
	}
...
func updateMySQLSchema(db *sqlx.DB) error {
	...
	_, err = db.Exec("ALTER TABLE `affiliations` ROW_FORMAT=DYNAMIC")
	if err != nil {
		return err
	}
