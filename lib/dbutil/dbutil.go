...
func createSQLiteDBTables(datasource string) error {
	...
	_, err = db.Exec(db.Rebind("INSERT INTO properties (property, value) VALUES ('identity', '0'), ('affiliation', '0'), ('certificate', '0')"))
	if err != nil {
...
func UpdateDBLevel(db *sqlx.DB, levels *Levels) error {
	...
	_, err := db.Exec(db.Rebind("UPDATE properties SET value = ? WHERE (property = 'identity')"), levels.Identity)
	if err != nil {
		return err
	}
	_, err = db.Exec(db.Rebind("UPDATE properties SET value = ? WHERE (property = 'affiliation')"), levels.Affiliation)
	if err != nil {
		return err
	}
	_, err = db.Exec(db.Rebind("UPDATE properties SET value = ? WHERE (property = 'certificate')"), levels.Certificate)
	if err != nil {
		return err
	}
