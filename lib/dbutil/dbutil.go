...
func NewUserRegistrySQLLite3(datasource string) (*sqlx.DB, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "Failed to create SQLite3 database")
	}
...
func NewUserRegistryPostgres(datasource string, clientTLSConfig *tls.ClientTLSConfig) (*sqlx.DB, error) {
	...
	log.Debug("Connecting to PostgreSQL server, using connection string: ", connStr)
	...
	log.Debugf("Connecting to database %s, using connection string: %s", dbName, datasource)
...
func createPostgresDatabase(dbName string, db *sqlx.DB) error {
	...
	_, err := db.Exec(query)
	fmt.Println("create pg db - err: ", err)
	if err != nil {
...
func NewUserRegistryMySQL(datasource string, clientTLSConfig *tls.ClientTLSConfig, csp bccsp.BCCSP) (*sqlx.DB, error) {
	...
	log.Debug("Connecting to MySQL server, using connection string: ", connStr)
	...
	log.Debugf("Connecting to database %s, using connection string: %s", dbName, connStr)
