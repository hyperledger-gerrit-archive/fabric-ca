...
func NewUserRegistrySQLLite3(datasource string) (*sqlx.DB, bool, error) {
	...
	if datasource != "" {
		...
		if err != nil && os.IsNotExist(err) {
			...
			if err2 != nil {
				return nil, false, errors.Wrap(err2, "Failed to create SQLite3 database")
			}
		} else {
...
func NewUserRegistryPostgres(datasource string, clientTLSConfig *tls.ClientTLSConfig) (*sqlx.DB, bool, error) {
	...
	if found == 0 {
		...
		if err != nil {
			return nil, false, errors.Wrap(err, "Failed to create Postgres database")
		}
	} else {
...
func NewUserRegistryMySQL(datasource string, clientTLSConfig *tls.ClientTLSConfig, csp bccsp.BCCSP) (*sqlx.DB, bool, error) {
	...
	if clientTLSConfig.Enabled {
		...
		if err != nil {
			return nil, false, errors.Wrap(err, "Failed to get client TLS for MySQL")
		}
	...
	if name == "" {
		...
		if err != nil {
			return nil, false, errors.Wrap(err, "Failed to create MySQL database")
		}
	} else {
		log.Debugf("Database (%s) exists", dbName)
		exists = true
