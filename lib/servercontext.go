...
// serverContext represents an HTTP request/response context in the server
...
func (ctx *serverContext) BasicAuthentication() (string, error) {
	r := ctx.req
...
func (ctx *serverContext) TokenAuthentication() (string, error) {
	r := ctx.req
	...
	if revokedOrExpired {
		return "", newErr(ErrRevokedOrExpired, 401, "Certificate was revoked or expired")
	}
	...
	for _, certificate := range certs {
		if certificate.Status == "revoked" {
			return "", newErr(ErrRevoked, 401, "Certificate was revoked")
		}
	}
	ctx.enrollmentID = id
...
func (ctx *serverContext) GetECert() *x509.Certificate {
	return ctx.enrollmentCert
}

// GetCA returns the CA to which this request is targeted
func (ctx *serverContext) GetCA() (*CA, error) {
	if ctx.ca == nil {
		// Get the CA name
...
func (ctx *serverContext) GetUserInfo(attrNames []string) ([]tcert.Attribute, []string, error) {
	id := ctx.enrollmentID
	if id == "" {
...
func (ctx *serverContext) getCAName() (string, error) {
...
func (ctx *serverContext) ReadBody(body interface{}) error {
...
func (ctx *serverContext) TryReadBody(body interface{}) (bool, error) {
...
func (ctx *serverContext) ReadBodyBytes() ([]byte, error) {
	if !ctx.body.read {
		r := ctx.req
