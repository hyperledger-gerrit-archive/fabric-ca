...
func BenchmarkGetCACert(b *testing.B) {
	...
	handler, err := newInfoHandler(srv)
	if err != nil {
		b.Fatalf("Failed to create getCACert request handler: %s", err)
	}
	chandler := handler.(*cfapi.HTTPHandler)
	for i := 0; i < b.N; i++ {
...
func BenchmarkRegister(b *testing.B) {
	...
	handler := registerHandler{server: srv}
	for i := 0; i < b.N; i++ {
		req, err := createRegisterRequest(admin, "user"+strconv.Itoa(i))
		if err != nil {
...
func BenchmarkEnroll(b *testing.B) {
	...
	admin := eresp.Identity
	handler, err := newEnrollHandler(srv)
	if err != nil {
		b.Fatalf("Failed to create enroll request handler: %s", err)
	}
	chandler := handler.(*cfapi.HTTPHandler)
	for i := 0; i < b.N; i++ {
...
func BenchmarkReenroll(b *testing.B) {
	...
	admin := eresp.Identity
	handler, err := newReenrollHandler(srv)
	if err != nil {
		b.Fatalf("Failed to create reenroll request handler: %s", err)
	}
	chandler := handler.(*cfapi.HTTPHandler)
	for i := 0; i < b.N; i++ {
		userName := "user" + strconv.Itoa(i)
...
func invokeRevokeBenchmark(b *testing.B) {
	...
	admin := eresp.Identity
	handler, err := newRevokeHandler(srv)
	if err != nil {
		b.Fatalf("Failed to create revoke handler: %s", err)
	}
	chandler := handler.(*cfapi.HTTPHandler)
	for i := 0; i < b.N; i++ {
		userName := "user" + strconv.Itoa(i)
		...
		err = chandler.Handle(rw, req)
		// if err == nil {
		// 	fmt.Println("revoke handler completed handling request successfully")
		// }
		b.StopTimer()
		if err != nil {
...
func createRevokeRequest(admin *Identity, user *Identity) (*http.Request, error) {
	...
	req.Header.Set(enrollmentIDHdrName, admin.GetName())
	return req, nil
}
...
func createReenrollRequest(user *Identity) (*http.Request, error) {
	...
	req.Header.Set(enrollmentIDHdrName, user.GetName())
	return req, nil
}
...
func createEnrollRequest(admin *Identity, userName string, regRes *api.RegistrationResponse) (*http.Request, error) {
	...
	req.Header.Set(enrollmentIDHdrName, userName)
	return req, nil
}
