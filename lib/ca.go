...
import (
	...
	cadbfactory "github.com/hyperledger/fabric-ca/lib/server/userregistry/db/factory"
...
func (ca *CA) initDB() error {
	...
	ca.certDBAccessor = NewCertDBAccessor(sqlxdb, ca.levels.Certificate)
	...
	err = ca.initUserRegistry(sqlxdb)
	if err != nil {
		return err
	}
	...
	migrator, err := getMigrator(ca.db, curLevels, ca.server.levels)
	if err != nil {
...
func (ca *CA) initUserRegistry(sqlxdb *sqlx.DB) error {
	...
	ca.registry = NewDBAccessor(sqlxdb)
...
func (ca *CA) DBAccessor() userregistry.UserRegistry {
	return ca.registry
}
...
func (ca *CA) getUserAffiliation(username string) (string, error) {
	...
	aff := cadbuser.GetUserAffiliation(user)
...
func getMigrator(db cadb.FabricCADB, curLevels, srvLevels *dbutil.Levels) (cadb.Migrator, error) {
