...
func (ca *CA) initIdemixKeyMaterial(renew bool) error {
	...
	idemixPubKey := ca.Config.CA.IssuerPublicKeyfile
	idemixSecretKey := ca.Config.CA.IssuerSecretKeyfile
...
func (ca *CA) getNewIssuerKey() (*idemix.IssuerKey, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrapf(err, "Error generating Idemix issuer key")
	}
...
func (ca *CA) getAttributeNames() []string {
	return []string{"OU", "Role", "EnrollmentID", "RevocationHandle"}
}
...
func (ca *CA) initConfig() (err error) {
	...
	if cfg.CA.IssuerPublicKeyfile == "" {
		cfg.CA.IssuerPublicKeyfile = "IssuerPublicKey"
	}
	if cfg.CA.IssuerSecretKeyfile == "" {
		cfg.CA.IssuerSecretKeyfile = "IssuerSecretKey"
	}
	if cfg.CSR.CA == nil {
	...
	if ca.Config.Cfg.RCOpts.RevocationHandlePoolSize == 0 {
		ca.Config.Cfg.RCOpts.RevocationHandlePoolSize = DefaultRevocationHandlePoolSize
	}
	return nil
}
...
// GetIdemixRand returns random number used by this CA in generation of nonces
// and Idemix credentials
func (ca *CA) GetIdemixRand() *amcl.RAND {
...
func (ca *CA) makeFileNamesAbsolute() error {
	...
	fields := []*string{
		...
		&ca.Config.CA.IssuerPublicKeyfile,
		&ca.Config.CA.IssuerSecretKeyfile,
	}
