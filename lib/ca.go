...
func (ca *CA) initKeyMaterial(renew bool) error {
	...
	if !renew {
		...
		if keyFileExists && certFileExists {
			...
			if err != nil {
				return errors.Wrap(err, "Validation of certificate and key failed")
			}
...
func (ca *CA) getCACert() (cert []byte, err error) {
	...
	if ca.Config.Intermediate.ParentServer.URL != "" {
		...
		if chainPath == "" {
			...
			if err != nil {
				return nil, errors.Wrap(err, "Failed to create intermediate chain file path")
			}
		...
		if err != nil {
			return nil, errors.Wrap(err, "Failed to create intermediate chain file")
		}
		...
		if err != nil {
			return nil, errors.Wrap(err, "Failed to create new CA certificate")
		}
	}
	return cert, nil
}
...
func (ca *CA) VerifyCertificate(cert *x509.Certificate) error {
	...
	if err != nil {
		return errors.Wrap(err, "Failed to get verify options")
	}
...
func (ca *CA) addIdentity(id *CAConfigIdentity, errIfFound bool) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to insert identity '%s'", id.Name)
	}
...
func (ca *CA) validateCert(certFile string, keyFile string) error {
	...
	if err = validateMatchingKeys(cert, keyFile); err != nil {
		return errors.Wrapf(err, "Invalid certificate and/or key in files '%s' and '%s'", certFile, keyFile)
	}
