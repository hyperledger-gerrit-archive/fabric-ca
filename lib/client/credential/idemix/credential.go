...
const (
	// CredentialType is the string that represents Idemix credential type
	CredentialType = "Idemix"
)
...
func (cred *Credential) CreateOAuthToken(reqBody []byte) (string, error) {
	...
	if err != nil {
		return "", errors.Wrapf(err, "Failed to unmarshal Idemix credential while creating oauth token")
	}
	cri := idemix.CredentialRevocationInformation{}
	err = proto.Unmarshal(cred.val.GetCredentialRevocationInformation(), &cri)
	if err != nil {
		return "", errors.Wrapf(err, "Failed to unmarshal Idemix CRI while creating oauth token")
	}

	sig, err := idemix.NewSignature(&credential, sk, nym, randNym, ipk, disclosure, digest, 3, &cri, rng)
	if err != nil {
		return "", errors.Wrapf(err, "Failed to create signature while creating oauth token")
	}
