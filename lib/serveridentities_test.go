...
func TestDynamicModifyIdentity(t *testing.T) {
	...
	modifyTypeAndAffiliation(t, admin2, admin3, modReq)
	modifyFixedValueAttrs(t, admin, modReq)
	modifyAttributes(t, registry, admin, admin3, modReq)
	modifySecretAndMaxEnroll(t, registry, admin, modReq)
...
func modifyTypeAndAffiliation(t *testing.T, admin2, admin3 *Identity, modReq *api.ModifyIdentityRequest) {
	modReq.ID = "testuser"
	modReq.Type = "user"
...
func modifyFixedValueAttrs(t *testing.T, admin *Identity, modReq *api.ModifyIdentityRequest) {
	modReq.Attributes = []api.Attribute{api.Attribute{
		Name:  attrEnrollmentID,
		Value: "nottestuser2",
	}}
	_, err := admin.ModifyIdentity(modReq)
	assert.Error(t, err, "Should have failed, can't modify 'hf.EnrollmentID' value to something besides the enrollment ID")
	...
	_, err = admin.ModifyIdentity(modReq)
	assert.Error(t, err, "Should have failed, can't modify 'hf.Type' value to something besides the identity's type")

	modReq.Attributes = []api.Attribute{api.Attribute{
		Name:  attrAffiliation,
		Value: "org1",
	}}
	_, err = admin.ModifyIdentity(modReq)
	assert.Error(t, err, "Should have failed, can't modify 'hf.Affiliation' value to something besides identity's affiliation")
}

func modifyAttributes(t *testing.T, registry spi.UserRegistry, admin, admin3 *Identity, modReq *api.ModifyIdentityRequest) {
	...
	// Should error, caller is allowed to modify value of this attribute but only its 'ecert' value.
	...
	_, err = admin.ModifyIdentity(modReq)
	assert.Error(t, err, "Should error, caller is allowed to modify value of this attribute but only its 'ecert' value")
	...
	_, err = admin.ModifyIdentity(modReq)
	assert.NoError(t, err, "Failed to update attribute's 'ecert' value")
	...
	if userTypeAttr.GetValue() == "orderer" {
		t.Error("Should not have been able to directly modify 'hf.Type', should only be updated if identity type is updated")
	}

	if userTypeAttr.ECert == false {
		t.Error("Failed to update 'ecert' boolean value to 'true'")
	}
	...
	assert.NoError(t, err, "Failed to get attribute 'hf.Type'")
...
func modifySecretAndMaxEnroll(t *testing.T, registry spi.UserRegistry, admin *Identity, modReq *api.ModifyIdentityRequest) {
