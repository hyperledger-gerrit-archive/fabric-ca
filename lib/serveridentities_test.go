...
func TestDynamicRemoveIdentity(t *testing.T) {
	...
	_, err = registry.GetUser(remReq.Name, nil)
	assert.Error(t, err, "User should not exist")

	certs, err := srv.CA.certDBAccessor.GetCertificatesByID(remReq.Name)
	if len(certs) == 0 {
		t.Errorf("No certificates found for '%s'", remReq.Name)
	}
	cert := certs[0]
...
func TestDynamicModifyIdentity(t *testing.T) {
	...
	val, err = user.GetAttribute("hf.Revoker")
	assert.NoError(t, err, "Failed to get attribute 'foo'")
	if val != "true" {
