...
func TestGetAllIDs(t *testing.T) {
	...
	os.Unsetenv("FABRIC_CA_CLIENT_GETIDS")
	result, err = captureOutput(admin2.GetAllIdentities, "")
	assert.NoError(t, err, "Failed to get all the appropriate identities")
...
func captureOutput(f func(caname string) error, caname string) (string, error) {
	old := os.Stdout
	r, w, err := os.Pipe()
	if err != nil {
		panic(err)
