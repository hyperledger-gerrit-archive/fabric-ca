...
func NewRevocationComponent(ca IdemixCA, level int) (*RevocationComponent, error) {
	...
	if err != nil {
		return nil, errors.WithMessage(err, fmt.Sprintf("Failed to initialize revocation component for CA %s", ca.GetConfig().CA.Name))
	}
	if rc.info.Epoch == 0 {
...
func (rc *RevocationComponent) getNextRevocationHandleTx(tx dbutil.FabricCATx, args ...interface{}) (interface{}, error) {
	...
	if err != nil {
		return nil, errors.New("Failed to get revocation component info from DB")
	}
	if len(rcInfos) == 0 {
		return nil, errors.New("No revocation component info found in DB")
	}
