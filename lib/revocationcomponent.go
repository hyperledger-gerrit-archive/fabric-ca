...
const (
	...
	// UpdateRCInfo is the SQL for updating revocation component info
	UpdateRCInfo = "UPDATE revocation_component_info SET epoch = ?, next_handle = ?, lasthandle_in_pool = ? WHERE (epoch = ?)"
...
// RevocationAuthority represents revocation authority that is responsible
// for generating revocation handles and credential revocation info (CRI)
type RevocationAuthority interface {
	GetNewRevocationHandle() (*RevocationHandle, error)
}

// RevocationComponentInfo is the information regarding revocation component
...
// RevocationComponentOptions encapsulates the configuration options for
// the Revocation Componanet
type RevocationComponentOptions struct {
	RevocationHandlePoolSize int `def:"100" help:"Specifies revocation handle pool size"`
}

// RevocationComponent implements RevocationAuthority interface
...
func NewRevocationComponent(ca IdemixCA, level int) (*RevocationComponent, error) {
	...
	if err == nil {
		...
		if rc.info.Epoch == 0 {
			rcInfo := RevocationComponentInfo{
				...
				LastHandleInPool:     ca.GetConfig().Cfg.RCOpts.RevocationHandlePoolSize,
				Level:                level,
			}
	...
	if err != nil {
		return nil, errors.WithMessage(err,
			fmt.Sprintf("Failed to initialize revocation component for CA %s", ca.GetConfig().CA.Name))
	}
	return rc, nil
}
...
func (rc *RevocationComponent) updateRCInfoInDB(epoch int, rcInfo *RevocationComponentInfo) error {
	res, err := rc.db.NamedExec(UpdateRCInfo, rcInfo)
	if err != nil {
		return errors.New("Failed to update revocation component info in database")
	}
	numRowsAffected, err := res.RowsAffected()
	if numRowsAffected != 1 {
		return errors.Errorf("Expected to affect 1 entry in revocation component info table but affected %d",
...
func (rc *RevocationComponent) getNextRevocationHandleTx(tx dbutil.FabricCATx, args ...interface{}) (interface{}, error) {
	...
	var inQuery string
	if newNextHandle == rcInfo.LastHandleInPool {
...
func (rc *RevocationComponent) doTransaction(db dbutil.FabricCADB, doit func(tx dbutil.FabricCATx, args ...interface{}) (interface{}, error), args ...interface{}) (interface{}, error) {
	...
	if err != nil {
		err2 := tx.Rollback()
		if err2 != nil {
			errMsg := fmt.Sprintf("Error encounted while rolling back transaction: %s, original error: %s", err2.Error(), err.Error())
			log.Errorf(errMsg)
