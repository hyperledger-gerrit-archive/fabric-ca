...
func TestGetCertificatesDB(t *testing.T) {
	...
	rows, err := ca.certDBAccessor.GetCertificates("testCertificate1", "", "", "", false, false, nil, nil, nil, nil)
	...
	rows, err = ca.certDBAccessor.GetCertificates("", "1111", "", "", false, false, nil, nil, nil, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, big.NewInt(1111), certs[0].SerialNumber)

	rows, err = ca.certDBAccessor.GetCertificates("", "", "9876", "", false, false, nil, nil, nil, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 2, len(certs))
	assert.Equal(t, []byte("9876"), certs[0].AuthorityKeyId)

	rows, err = ca.certDBAccessor.GetCertificates("", "", "", "", true, false, nil, nil, nil, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 5, len(certs))

	rows, err = ca.certDBAccessor.GetCertificates("", "", "", "", false, true, nil, nil, nil, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 6, len(certs))

	rows, err = ca.certDBAccessor.GetCertificates("", "", "", "dept1", false, false, nil, nil, nil, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 1, len(certs))
	assert.Equal(t, "testCertificate1", certs[0].Subject.CommonName)

	rows, err = ca.certDBAccessor.GetCertificates("", "", "9876AB", "", false, false, nil, nil, nil, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 1, len(certs))
	...
	rows, err = ca.certDBAccessor.GetCertificates("", "", "", "", false, false, &revokedStart, nil, nil, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 1, len(certs))
	...
	rows, err = ca.certDBAccessor.GetCertificates("", "", "", "", false, false, nil, &revokedEnd, nil, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 2, len(certs))
	...
	rows, err = ca.certDBAccessor.GetCertificates("", "", "", "", false, false, &revokedStart, &revokedEnd, nil, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 1, len(certs))
	...
	rows, err = ca.certDBAccessor.GetCertificates("", "", "", "", false, false, nil, nil, &expiredStart, nil)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 6, len(certs))

	expiredEnd := time.Date(2018, time.July, 1, 0, 0, 0, 0, time.UTC)
	rows, err = ca.certDBAccessor.GetCertificates("", "", "", "", false, false, nil, nil, nil, &expiredEnd)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	...
	rows, err = ca.certDBAccessor.GetCertificates("", "", "", "", false, false, nil, nil, &expiredStart, &expiredEnd)
	assert.NoError(t, err, "Failed to get certificates from database")
	certs, err = readRows(rows)
	assert.Equal(t, 1, len(certs))
