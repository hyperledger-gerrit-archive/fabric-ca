...
func (d *CertDBAccessor) GetRevokedCertificates(curTime, after, before time.Time) ([]certdb.CertificateRecord, error) {
	...
	err = d.db.Select(&crs, fmt.Sprintf(d.db.Rebind(selectRevokedAndUnexpiredSQL),
		sqlstruct.Columns(certdb.CertificateRecord{})), curTime, after, before)
	if err != nil {
		return crs, dbGetError(err, "Certificate")
	}
	return crs, nil
