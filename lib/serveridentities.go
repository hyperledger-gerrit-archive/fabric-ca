...
func processStreamingRequest(ctx *serverRequestContextImpl, caname string, caller userregistry.User) (interface{}, error) {
...
func processRequest(ctx *serverRequestContextImpl, caname string, caller userregistry.User) (interface{}, error) {
	log.Debug("Processing identity configuration update request")
...
func processGetAllIDsRequest(ctx *serverRequestContextImpl, caller userregistry.User, caname string) error {
...
func processGetIDRequest(ctx *serverRequestContextImpl, caller userregistry.User, caname string) (interface{}, error) {
...
func getIDs(ctx *serverRequestContextImpl, caller userregistry.User, caname string) error {
...
func getID(ctx *serverRequestContextImpl, caller userregistry.User, id, caname string) (*api.GetIDResponse, error) {
...
func getModifyReq(user userregistry.User, req *api.ModifyIdentityRequest) (*cadbuser.Info, bool) {
	modifyUserInfo := user.(*dbuser.User).Info
	userPass := user.(*dbuser.User).GetPass()
	setPass := false
	...
	if len(reqAttrs) != 0 {
		modifyUserInfo.Attributes = dbuser.GetNewAttributes(modifyUserInfo.Attributes, reqAttrs)
	}
...
func getIDResp(user userregistry.User, secret, caname string) (*api.IdentityResponse, error) {
	allAttributes, err := user.GetAttributes(nil)
	if err != nil {
		return nil, err
	}
