...
func processGetRequest(ctx *serverRequestContext, caller spi.User, caname string) (interface{}, error) {
	...
	if vars == nil {
		return nil, newHTTPErr(400, ErrHTTPRequest, "Failed to correctly handle HTTP request")
	}
	id = vars["id"]
...
func getAllIDs(ctx *serverRequestContext, caller spi.User, caname string) (*api.GetAllIDsResponse, error) {
	...
	ids, err := registry.GetUsersByAffiliation(callerAff)
	if err != nil {
