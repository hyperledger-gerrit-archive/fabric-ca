...
func getIDs(ctx *serverRequestContext, caller spi.User, caname string) error {
	...
	if numberOfIdentities == "" {
		...
		if err != nil {
			return newHTTPErr(400, ErrGettingUser, "Incorrect format specified for number of rows, use an integer value: %s", err)
		}
	}
	...
	for i := 0; i < numIdentities; i++ {
		for rows.Next() {
			...
			resp, _ := util.Marshal(idInfo, "identities info")
			w.Write(resp)
		}
