...
//	//Fail cases
//	server.HomeDir = ""
//	fileInfo, _ := os.Stat(os.Getenv("PWD"))
//	oldmode := fileInfo.Mode()
//	err = os.Chmod(os.Getenv("PWD"), 0000)
//	if err != nil {
//		t.Fatal("Chmod on PWD failed")
//	}
//	err = server.Init(false)
...
//	}
//	_ = os.Chmod(os.Getenv("PWD"), oldmode)
}
...
func TestSRVDefaultAddrPort(t *testing.T) {
	...
	l, err := net.Listen("tcp4", "0.0.0.0:7054")
	if err != nil {
		t.Fatal("Failed to create tcp socket")
	}
	t.Logf("opened tcp listener on %s", l.Addr().String())
	...
	if err == nil {
		t.Fatalf("Root server start should have failed (port unavailable")
	}
...
func TestSRVMultiCAConfigs(t *testing.T) {
	...
	if !util.FileExists("../testdata/ca/rootca/ca1/ca-key.pem") {
		t.Error("../testdata/ca1/ca-key.pem doesn't exist")
	}
	...
	if err = os.Chmod("../testdata/ca/rootca/ca1/ca-key.pem", 0000); err != nil {
		t.Error("Failed to chmod file: ../testdata/ca1/ca-key.pem, %v", err)
	}
	err = ioutil.WriteFile("../testdata/ca/rootca/ca1/ca-cert.pem", make([]byte, 1), 0644)
	if err = os.Chmod("../testdata/ca/rootca/ca1/ca-cert.pem", 0000); err != nil {
		t.Error("Failed to chmod file: ../testdata/ca/rootca/ca1/ca-cert.pem, %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	err = srv.Start()
	t.Logf("srv.Start ERROR %v", err)
	...
	err = os.Link("../testdata/expiredcert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink expired cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/tls_client-key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Error("symlink key to ../testdata/ca/rootca/ca1/ca-key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	err = srv.Start()
	t.Logf("srv.Start ERROR %v", err)
	if err == nil {
		t.Error("Should have failed to start server, expired cert")
	}
	err = os.Remove("../testdata/ca/rootca/ca1/ca-key.pem")
	err = os.Remove("../testdata/ca/rootca/ca1/ca-cert.pem")

	// Starting server with invalid usage
	err = os.Link("../testdata/noKeyUsage.cert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink invalid cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/noKeyUsage.key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Error("symlink key to ../testdata/tls_client-key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	err = srv.Start()
	t.Logf("srv.Start ERROR %v", err)
	if err == nil {
		t.Error("Should have failed to start server, invalid usage cert")
	}
	err = os.Remove("../testdata/ca/rootca/ca1/ca-key.pem")
	err = os.Remove("../testdata/ca/rootca/ca1/ca-cert.pem")

	// Starting server with basic constraint, CA:FALSE
	err = os.Link("../testdata/caFalse.cert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink non-CA cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/caFalse.key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Error("symlink key to ../testdata/caFalse.key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	err = srv.Start()
	t.Logf("srv.Start ERROR %v", err)
	...
	// Starting server with invalid key type
	err = os.Link("../testdata/dsaCa-cert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink dsa cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/dsaCa-key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Error("symlink key to ../testdata/ca/rootca/ca1/ca-key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	...
	t.Logf("srv.Start ERROR %v", err)
	if err == nil {
		t.Error("Should have failed to start server, invalid key type")
	}
	err = os.Remove("../testdata/ca/rootca/ca1/ca-key.pem")
	err = os.Remove("../testdata/ca/rootca/ca1/ca-cert.pem")

	// Starting server with invalid key size
	err = os.Link("../testdata/rsa512-cert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink dsa cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/rsa512-key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Error("symlink key to ../testdata/ca/rootca/ca1/ca-key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	...
	t.Logf("srv.Start ERROR %v", err)
	if err == nil {
		t.Error("Should have failed to start server, invalid key size")
	}
	err = os.Remove("../testdata/ca/rootca/ca1/ca-key.pem")
	err = os.Remove("../testdata/ca/rootca/ca1/ca-cert.pem")

	// Starting server with non-matching ecdsa keys
	err = os.Link("../testdata//ec256-1-cert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink dsa cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/ec256-2-key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Error("symlink key to ../testdata/ca/rootca/ca1/ca-key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	...
	t.Logf("srv.Start ERROR %v", err)
	if err == nil {
		t.Error("Should have failed to start server, non-matching keys")
	}
	err = os.Remove("../testdata/ca/rootca/ca1/ca-key.pem")
	err = os.Remove("../testdata/ca/rootca/ca1/ca-cert.pem")

	// Starting server with non-matching rsa keys
	err = os.Link("../testdata/rsa2048-1-cert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink dsa cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/rsa2048-2-key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Error("symlink key to ../testdata/ca/rootca/ca1/ca-key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	...
	// Starting server with non-matching rsa keys
	err = os.Link("../testdata/unRipeCaEc256-cert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink dsa cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/unRipeCaEc256-key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Error("symlink key to ../testdata/ca/rootca/ca1/ca-key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	...
	// Starting server with bad ec keys
	err = os.Link("../testdata/ec256-1-cert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink dsa cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/rsa2048-1-cert.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Error("symlink key to ../testdata/ca/rootca/ca1/ca-key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	...
	err = os.Link("../testdata/ec256-1-cert.pem", "../testdata/ca/rootca/ca1/ca-cert.pem")
	if err != nil {
		t.Error("symlink ec256-1 cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
