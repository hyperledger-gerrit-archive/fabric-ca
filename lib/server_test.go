...
func TestSRVMultiCAConfigs(t *testing.T) {
	...
	if err != nil {
		t.Errorf("symlink expired cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	...
	if err != nil {
		t.Errorf("symlink invalid cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/noKeyUsage.key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Errorf("symlink key to ../testdata/tls_client-key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	err = srv.Start()
	t.Logf("srv.Start ERROR %v", err)
	...
	if err != nil {
		t.Errorf("symlink non-CA cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
	err = os.Link("../testdata/caFalse.key.pem", "../testdata/ca/rootca/ca1/ca-key.pem")
	if err != nil {
		t.Errorf("symlink key to ../testdata/caFalse.key.pem failed %v", err)
	}
	srv.Config.CAfiles = []string{"ca/rootca/ca1/fabric-ca-server-config.yaml"}
	err = srv.Start()
	t.Logf("srv.Start ERROR %v", err)
	...
	if err != nil {
		t.Errorf("symlink dsa cert to ../testdata/ca1/ca-cert.pem failed %v", err)
	}
