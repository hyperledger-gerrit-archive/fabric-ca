...
func TestRootServer(t *testing.T) {
	...
	if assert.Error(t, err) {
		assert.Contains(t, err.Error(), "is set to false")
	}
...
func TestIntermedaiteServerWithFalseAttr(t *testing.T) {
	var err error
	...
	if assert.Error(t, err, "Should have failed, the attribute 'hf.IntermediateCA' is set to false. Cannot register as Intermediate CA") {
		assert.Contains(t, err.Error(), "is set to false")
	}
	// Stop it
