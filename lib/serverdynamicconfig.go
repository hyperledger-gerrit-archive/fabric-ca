...
func addAffiliation(affiliation string, ctx *serverRequestContext) (string, error) {
	addAff := strings.Split(affiliation, ":")
	log.Debugf("Adding affiliation '%s'", addAff)

	err := insertAff(addAff[1], ctx)
	if err != nil {
		return "", newHTTPErr(400, ErrUpdateConfigAddAff, "Failed to add affiliation: %s", err)
	}
...
func insertAff(affiliation string, ctx *serverRequestContext) error {
	affiliations := strings.Split(affiliation, ".")

	var affilationPath string
	var parentAffiliationPath string

	for _, aff := range affiliations {
		affilationPath = affilationPath + aff
		err := ctx.ca.registry.InsertAffiliation(affilationPath, parentAffiliationPath)
		if err != nil {
			return err
		}
		parentAffiliationPath = affilationPath
		affilationPath = affilationPath + "."
	}

	return nil
}

func aggregateMessages(action string, allErrors string, msg string) string {
	errStr := fmt.Sprintf("'%s' = %s", action, msg)
	if allErrors == "" {
		allErrors = errStr
	} else {
