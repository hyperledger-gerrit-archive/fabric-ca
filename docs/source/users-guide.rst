...
To upgrade a single instance of Fabric CAserver:

1. Stop the fabric-ca-server process.
2. If using sqlite3, backup the current database file (which is named fabric-ca-server.db 
   by default). For other database types, use the appropriate backup/replication mechanism.  
...
3) Install upgraded binaries of fabric-ca-server on host3 and host4. The new
   ...
   Also, haproxy ``"show stat"`` should now reflect that all servers are active,
