...
If you don't have a certificate and key to use for TLS, they may be generated using OpenSSL as
shown below.

1. Navigate to a directory where you would like to store the server's certificate and key
...
If a Fully Qualified Domain Name (FQDN) is not going to be used for the CN than it necessary to
specify hostname(s) in the x509 Subject Alternative Names (SAN) extension which allows multiple
hostnames and/or IP addresses to be used.
...
may specify the FQDN of the host on which the certificate will be used.
...
and key. If you don't have a certificate and key to use for TLS, they may be generated using
OpenSSL as shown below.

1. Navigate to a directory where you would like to store the client's certificate and key
...
.. code:: yaml
    ...
    tls:
      ...
      certfiles:
        - root.pem
      client:
        certfile: client-tls-cert.pem
        keyfile: client-tls-key.pem
