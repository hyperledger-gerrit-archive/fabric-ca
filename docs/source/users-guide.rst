...
By default, the Fabric CA server returns the CA chain in child-first order, which means, each CA
certificate in the chain is followed by issuer's CA certificate. If you need the Fabric CA server
to return the CA chain in reverse order, then set the environment variable ``CA_CHAIN_PARENT_FIRST``
to any value and restart the Fabric CA server. The Fabric CA client will handle both the formats.
