...
`Org1`, `Dept1`, `Org2` are stored in lower case. This is because Fabric CA uses Viper to read configuration.
It treats map keys as case insensitive and always returns lowercase value. To register an identity with
`Unit1` affiliation, `org1.dept1.Unit1` would need to be specified to the
`--id.affiliation` flag as shown below:

.. code:: bash

    export FABRIC_CA_CLIENT_HOME=$HOME/fabric-ca/clients/admin
    fabric-ca-client register --id.name client1 --id.type client --id.affiliation org1.dept1.Unit1
