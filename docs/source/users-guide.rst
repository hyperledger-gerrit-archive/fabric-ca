...
.. code:: yaml
    ...
    ldap:
       ...
       attribute:
          ...
          # requested with the LDAP server is queried to login in a user.
          names: <LDAPAttrs>
...
Where:
  ...
  * the attribute.converters section is used to convert LDAP attributes to fabric
    ...
    * ``ldapExpr`` is an LDAP expression whose evaluated value is assigned to
      the fabric CA attribute.
    For example, suppose <LDAPAttrs> is ["uid"], <fcaAttrName> is 'hf.Revoker',
    and <fcaExpr> is 'attr("uid") =~ "revoker*"'.  An attribute named "uid"
    is requested from the LDAP server on a user's behalf.  The user is then
    given a value of 'true' for the 'hf.Revoker' attribute if the value of
