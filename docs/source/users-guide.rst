...
The version of the CRI is increased by one when all the revocation handles in the revocation handle pool are used. The size of the revocation
handle pool is set by the ``idemix.rhpoolsize`` server configuration property. When all handles in the pool are exhausted, pool is 
populated with new set of revocation handles and the CRI version is incremented.
