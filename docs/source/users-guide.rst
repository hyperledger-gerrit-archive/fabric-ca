...
5. Verify the fabric-ca-server process is available with the following command where <host> is the hostname on which the server was started::
      ...
      fabric-ca-client getcacert -u http://<host>:7054 --tls.certfiles tls-cert.pem

Upgrading a cluster:
^^^^^^^^^^^^^^^^^^^^
To upgrade a cluster of fabric-ca-server instances using either a MySQL or Postgres database, perform the following procedure. We assume that you are using haproxy to load balance to two fabric-ca-server v1 cluster members on host1 and host2, respectively, both listening on port 7054. After this procedure, you will be load balancing to to fabric-ca-server v1.1 cluster members on host3 and host4, respectively, both listening on port 7054.
...
2) Restart the HA proxy with the new configuration as follows::
   ...
   haproxy ``"show stat"`` will now reflect the modified configuration,
   with two active v1.0 backup servers and two (yet to be started) v1.1 servers::
...
3) Install v1.1 servers on host3 and host4 and start them. Both of these
   servers should be configured to use the same database as the fabric-ca-server
   instances on host1 and host2. After starting these v1.1 servers, the database
   will be automatically migrated and the haproxy configuration will forward all
   new traffic to the v1.1 servers which are not marked as backup servers.
   Verify using the ``"fabric-ca-client getcacert"`` command that your cluster
   is still functioning appropriately before proceeding. Also, haproxy
   ``"show stat"`` should now reflect that all servers are active,
   similar to the following::

      sid   pxname      svname  status  weig  act  bck
        1   fabric-cas  server3    UP     1    1    0
        2   fabric-cas  server4    UP     1    1    0
        3   fabric-cas  server0    UP     1    0    1
        4   fabric-cas  server1    UP     1    0    1

4) Stop the v1.0 servers on host1 and host2. Verify using the
   ``"fabric-ca-client getcacert"`` command that your cluster is still
   functioning appropriately before proceeding. Then delete the v1.0 servers.

   Remove the v1.0 servers from the haproxy configuration file,
