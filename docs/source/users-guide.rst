...
An authorization failure will occur if the client identity does not satisfy all of the following:
 ...
 - The affiliation of the client's identity must be equal to or a prefix of
    ...
    should be a dot (".") and the client must also have root affiliation.
...
.. code:: bash
    ...
    fabric-ca-client servercfg add 'registry.identities={"id": "user1", "secret": "user1pw", "type": "user", "affiliation": "org1", "maxenrollments": 1, "attrs": [{"name:": "hf.Revoker", "value": "true"}]}'
