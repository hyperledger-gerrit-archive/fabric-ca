...
4. Suppose an error similar to
   ...
   is returned by a peer or orderer when using an enrollment certificate issued by a `fabric-ca-server`.  This indicates that
   the signing CA certificate used by `fabric-ca-server` to issue certificates does not match a certificate in the `cacerts` or `intermediatecerts`
   ...
   b. You used deleted and recreated the CA signing key and certificate used by `fabric-ca-server` after generating the genesis block.
      This can happen if you `fabric-ca-server` is running in a docker container, the container was restarted, and its home directory
      is not on a volume mount.  In this case, the `fabric-ca-server` will create a new CA signing key and certificate.

      Assuming you can not recover the original CA signing key, the only way to recover from this situation is to update the
