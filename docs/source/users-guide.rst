...
If you don't have a certificate and key to use for TLS, they maybe generated using openSSL. Instructions
below guide you on how to generate a self-signed certifcate for TLS.

1. Navigate to a directory where you would like to store the server certificate and key pair
2. Run the following: 

::

    openssl req -x509 -newkey rsa:4096 -keyout server-tls-key.pem -out server-tls-cert.pem -days 365 -nodes

3. Respond to prompts with appropriate answers for the CSR. For the Common Name (CN) prompt, it is
necessary to use the FQDN of the host on which the certificate will be used.

This will generate a certificate and key pair that can now used in the server side TLS configuration.
...
that the server trusts and will be used to verify client side TLS certficates.
...
If the Fabric CA has mutual TLS enabled then client will need a TLS certificate
and key. If you don't have a certificate and key to use for TLS, they maybe generated
using openSSL. Instructions below guide you on how to generate a self-signed certifcate
for TLS use.

1. Navigate to a directory where you would like to store the client certificate and key pair
2. Run the following: 

::

    openssl req -x509 -newkey rsa:4096 -keyout tls-client-key.pem -out tls-client-cert.pem -days 365 -nodes

3. Respond to prompts with appropriate answers for the CSR. For the Common Name (CN) prompt, it is
necessary to use the FQDN of the host on which the certificate will be used.

This will generate a certificate and key pair that can be used in the client side TLS configuration.
...
client. The **client** option is required only if mutual TLS is configured on
the server.
