...
The TLS CA's signing certificate will need to available on each host that will run
...
identities. In the commands below, we will assume the issuing certificate for CA's
TLS certificate has been copied to /tmp/hyperledger/tls-ca/crypto/tls-ca-cert.pem
...
`
In the commands below, we will assume the signing certificate for the CA's
certificate has been copied to /tmp/hyperledger/org0/ca/crypto/ca-cert.pem
...
`
In the commands below, we will assume the signing certificate for the CA's
certificate has been copied to /tmp/hyperledger/org1/ca/crypto/ca-cert.pem
on host machine where the fabric-ca-client binary is present.
Acquiring of the signing certificate might be out of band process, if the client
binary is located on a different host.
...
go through the set of steps that administrator for org2 will perform.
...
related identities registered. In the commands below, we will assume the signing
certificate of org2's CA's has been copied to
...
Once the CAs are up and running, we are the point that we can start enrolling
peers.
...
In the command below, we will assume the signing certificate of org1's CA has
...
folder. This folder will contain certificates for the administrator of org1.
...
assume the signing certificate for org1's CA has been has been copied to
...
At this point, both peer identities have been enrolled. Now, you will enroll
...
The command below assumes that this is being executed in on peer1's host machine.
...
we will assume the the sigining certificate of Org2's CA is available at
...
below, we will assume the signing certificate for Org0's CA's is available a
...
At this point, both peer identities have been enrolled. Now, you will enroll
Org0's admin identity. The steps below will enroll the admin. The command below
assumes that this is being executed in on orderer's host machine.
...
You will copy the certifcate from this MSP and move it to the peer MSP under the
``admincerts`` folder.
...
The orderer requires a genesis block that the orderer uses to bootstrap itself.
...
In this example, you'll find an example configtx.yaml that is written for this
specific deployment. 

On the orderer host machine, we need to collect the MSPs for all the
...
The MSP for org0 will contain the signing certificate from Org0's CA,
the certificate of the Org0's admin identity, and the signing certificate from
...
This same pattern follows for all organization MSPs. The MSP folder structure for
Org1 would like:
...
.. code:: bash
   ...
   genesis block: configtxgen -profile OrgsOrdererGenesis -outputBlock /tmp/hyperledger/orderer/genesis.block
   channel block: configtxgen -profile OrgsChannel -outputCreateChannelTx /tmp/hyperledger/orderer/channel.tx -channelID mychannel

Launch Orderer
^^^^^^^^^^^^^^^

Once you have created the genesis block and the channel transaction you can
start the orderer server.
...
Launching the orderer service will bring up a orderer container, and in the logs you will
see the following line:
...
Communication with peers requires a CLI container that has the appropriate
binaries that will allow you to issue peer releated commands. You will create
a CLI container for each org. In this example, we launch the CLI container
in the same host machine as peer1.
...
.. code:: yaml
 ...
 cli-org2:
    ...
    environment:
      ...
      - CORE_PEER_LOCALMSPID=org2MSP
      - CORE_PEER_TLS_ENABLED=true
      - CORE_PEER_TLS_ROOTCERT_FILE=/tmp/hyperledger/org2/peer/tls-msp/tlscacerts/tls-0-0-0-0-7052.pem
      - CORE_PEER_MSPCONFIGPATH=/tmp/hyperledger/org2/peer2/msp
    working_dir: /opt/gopath/src/github.com/hyperledger/fabric/
    command: sh 
    volumes:
...
join a channel. We are going to use peer1-org1 to to create the channel. In the
host machine of peer1-org1, you will execute:

.. code:: bash

   docker exec -it cli-org1 sh

This command will bring you inside the container and open up a terminal. From
...
to peer1-org1 host out of band. This command above will generate
``mychannel.block`` that will be used by all peers in the network that wish
...
need to create the channel. From inside the Org2 CLI container, you will execute
the following commands using the admin MSP:
...
On Peer1 in Org1, you are going to install chaincode. The command assume that the
