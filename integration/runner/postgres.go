...
func (c *PostgresDB) verifyImageAvailable() error {
	ioReader, err := c.Client.ImagePull(context.Background(), c.Image, types.ImagePullOptions{})
	if err != nil {
		return err
	}
...
func (c *PostgresDB) Stop() error {
	...
	err := c.Client.ContainerStop(context.Background(), c.containerID, nil)
	if err != nil {
		return err
	}
