...
build/image/fabric-ca/payload: build/docker/bin/fabric-ca build/sampleconfig.tar.bz2 build/certfiles.tar.bz2
build/image/fabric-ca-fvt/payload: build/docker/bin/fabric-ca images/fabric-ca-fvt/start.sh
build/image/fabric-ca-client/payload: build/docker/bin/fabric-ca-client build/fabric-ca-client.tar.bz2
build/image/fabric-ca-server/payload: build/docker/bin/fabric-ca-server build/fabric-ca-server.tar.bz2
build/image/%/payload:
	mkdir -p $@
	cp $^ $@

build/sampleconfig.tar.bz2: $(shell git ls-files images/fabric-ca/config)
	@echo "Building $@"
	@tar -jc -C images/fabric-ca/config $(notdir $^) > $@

build/certfiles.tar.bz2: $(shell git ls-files images/fabric-ca/certs)
	@echo "Building $@"
	@tar -jc -C images/fabric-ca/certs $(notdir $^) > $@

build/fabric-ca-client.tar.bz2: $(shell git ls-files images/fabric-ca-client/payload)
build/fabric-ca-server.tar.bz2: $(shell git ls-files images/fabric-ca-server/payload)
build/%.tar.bz2:
	@echo "Building $@"
