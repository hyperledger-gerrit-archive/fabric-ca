...
func (c *revokeCmd) runRevokeAll(cmd *cobra.Command, args []string) error {
	...
	if c.args.GenCRL {
		return storeCRL(c.GetClientCfg(), []byte(result.RevocationResponse.CRL))
	}

	log.Infof("Sucessfully revoked certificates: %+v\nSuccessfully revoked credential: %+v", result.RevocationResponse.RevokedCerts, result.IdemixRevocation.RevokedHandles)
	return nil
}
