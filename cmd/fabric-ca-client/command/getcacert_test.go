...
func TestNewGetCACertCmd(t *testing.T) {
	...
	assert.Equal(t, getcacertCmd.Name(), "getcacert")
	assert.Equal(t, getcacertCmd.Short, GetCACertCmdShortDesc)
	assert.Equal(t, getcacertCmd.Use, GetCACertCmdUsage)
}
...
func TestPreRunGetCACertBadConfig(t *testing.T) {
	...
	getcacertCmd := NewGetCACertCmd(cmd)
	err := getcacertCmd.PreRunE(getcacertCmd, []string{})
	assert.Error(t, err)
