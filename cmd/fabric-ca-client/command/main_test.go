...
func TestGenCRL(t *testing.T) {
	...
	client := &lib.Client{
		Config:  &lib.ClientConfigImpl{URL: fmt.Sprintf("http://localhost:%d", serverPort)},
		HomeDir: adminHome,
	}
...
func TestAffiliationCmd(t *testing.T) {
	...
	assert.Equal(t, result, "affiliation: org1\n")
...
func registerAndRevokeUsers(t *testing.T, admin *lib.Identity, num int) []*big.Int {
	...
	for i := 0; i < num; i++ {
		...
		signer := enrollResp.Identity.GetX509Credential()
		if signer == nil || signer.GetX509Cert() == nil {
			t.Fatalf("Failed to get enrollment certificate for the user %s", userName)
		}
		cert := signer.GetX509Cert()
