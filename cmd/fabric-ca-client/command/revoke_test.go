...
func TestRevokeIdemixError(t *testing.T) {
	...
	revoke.On("RevokeIdemix", &api.RevocationRequest{}).Return(nil, errors.New("Failed to revoke idemix credential"))
...
func TestRevokeIdemixPass(t *testing.T) {
	...
	rCmd := newRevokeCmd(cmd)
	revoke := new(mocks.Revoke)
	revoke.On("RevokeIdemix", &api.RevocationRequest{}).Return(&idemixapi.RevocationResponse{}, nil)
	_, err := rCmd.revokeIdemix(revoke)
...
func (g *mockGoodIdentity) RevokeIdemix(*api.RevocationRequest) (*idemixapi.RevocationResponse, error) {
	return &idemixapi.RevocationResponse{}, nil
}
