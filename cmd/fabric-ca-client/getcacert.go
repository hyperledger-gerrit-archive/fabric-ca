...
func storeCAChain(config *lib.ClientConfig, si *lib.GetServerInfoResponse) error {
	...
	err = storeFile("CA root certificate", mspDir, "cacerts", fname, rootCert)
	if err != nil {
		return err
	}
	// Store the intermediate certs if there are any
	if len(intermediateCerts) > 0 {
		err = storeFile("CA intermediate certificates", mspDir, "intermediateCerts", fname, intermediateCerts)
		if err != nil {
			return err
		}
	}
