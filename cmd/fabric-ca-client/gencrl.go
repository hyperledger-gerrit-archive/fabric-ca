...
func (c *ClientCmd) newGenCRLCommand() *cobra.Command {
	...
	genCrlCmd.Flags().StringVar(&c.crlParams.revokedAfter, "revokedafter", "",
		"Generate CRL with unexpired certificates that were revoked after this UTC timestamp (in RFC3339 format)")
	genCrlCmd.Flags().StringVar(&c.crlParams.revokedBefore, "revokedbefore", "",
		"Generate CRL with unexpired certificates that were revoked before this UTC timestamp (in RFC3339 format)")
