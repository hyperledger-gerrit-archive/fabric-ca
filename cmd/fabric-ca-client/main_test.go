...
func removeAffiliationsAllowed(t *testing.T, db spi.UserRegistry) {
	...
	err = RunMain([]string{cmdName, "register", "--id.name", "testuser", "--id.affiliation", "org2.dept1"})
	...
	err = RunMain([]string{cmdName, "servercfg", "--force", "remove", "affiliations.org2.dept1"})
	assert.NoError(t, err, "Failed to remove affiliation")
