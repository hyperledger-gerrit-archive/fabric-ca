...
func TestGenCRL(t *testing.T) {
	...
	os.Setenv("FABRIC_CA_CLIENT_HOME", tmpHome)
	...
	// success case 3: gencrl invoked with --revokedbefore argument but not --revokedafter
	futureTime := time.Now().UTC().Add(time.Hour * 1).Format(time.RFC3339)
	err = RunMain([]string{cmdName, "gencrl", "--revokedbefore", futureTime})
	assert.NoError(t, err, "gencrl failed")
	checkCRL(t, admin.GetClient(), revokedCertSerials)

	// success case 4: gencrl invoked with --expirebefore, --revokedbefore and --revokedafter args
	...
	err = RunMain([]string{cmdName, "gencrl", "--revokedafter", pastTime,
		"--revokedbefore", futureTime, "--expireafter", expTime})
	assert.Error(t, err, "gencrl should have failed as there no revoked certs that expire 3 hours from now")
	...
	// Error case 5: should fail when invoked with invalid --revokedafter arg
	...
	// Error case 7: should fail when invoked with invalid --revokedbefore arg
	...
	// Error case 8: should fail when invoked with revokeafter value is greater (comes after) than revokedbefore
	...
	// Error case 9: should fail when invoked with invalid --expireafter arg
	...
	// Error case 10: should fail when invoked with invalid --expireafter arg
	...
	// Error case 11: should fail when invoked with invalid --expirebefore arg
	...
	// Error case 12: should fail when invoked with invalid --expirebefore arg
