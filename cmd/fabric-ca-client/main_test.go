...
func TestEnroll(t *testing.T) {
	dir, _ = getDefaultConfigFile()

	// Negative test case, enroll command without username/password
	err := RunMain([]string{cmdName, "enroll", "-d", "-s", "http://localhost:7054"})
	fmt.Println("err: ", err)
	if err == nil {
	...
	err = RunMain([]string{cmdName, "enroll", "-s", "http://localhost:7054", "-u", "foo:bar"})
	if err != nil {
		t.Errorf("client enroll -s -u failed: %s", err)
	}
...
func TestReenroll(t *testing.T) {
	...
	err = RunMain([]string{cmdName, "reenroll", "-s", "http://localhost:7054"})
	if err != nil {
		t.Errorf("client reenroll -s failed: %s", err)
	}
...
func TestRegister(t *testing.T) {
	...
	err = RunMain([]string{cmdName, "register", "-s", "http://localhost:7054", "-f", "regRequest.json"})
	if err != nil {
		t.Errorf("client register -s -f failed: %s", err)
	}
...
func TestRevoke(t *testing.T) {
	...
	err = RunMain([]string{cmdName, "revoke", "-s", "http://localhost:7054"})
	if err != nil {
		t.Errorf("client revoke -s failed: %s", err)
	}
