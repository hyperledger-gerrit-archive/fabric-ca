...
func TestClientCommandsNoTLS(t *testing.T) {
	...
	srv = lib.GetServer(serverPort, testdataDir, "", -1, t)
	srv.HomeDir = tdDir
	...
	aff["hyperledger"] = []string{"bank_a", "bank_b", "bank_c"}
	aff["hyperledger2"] = []string{}
...
func testRegisterEnvVar(t *testing.T) {
	...
	os.Setenv("FABRIC_CA_CLIENT_ID_AFFILIATION", "hyperledger2")
...
func testRegisterCommandLine(t *testing.T) {
	...
	err := RunMain([]string{cmdName, "register", "-d", "--id.name", "testRegister3", "--id.affiliation", "hyperledger.bank_a", "--id.type", "client", "--id.attr", "hf.test=a=b"})
	if err != nil {
