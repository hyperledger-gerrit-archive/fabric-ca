...
func TestAffiliationCmd(t *testing.T) {
	...
	if assert.Error(t, err, "Should have failed, too many arguments") {
		assert.Contains(t, err.Error(), "Too many arguments, only the affiliation name should be passed in as argument")
	}
	...
	err = RunMain([]string{
		cmdName, "affiliation", "modify", "org2", "--name", "org4"})
	assert.Error(t, err, "Failed to rename affiliation from 'org2' to 'org3'")

	_, err = registry.GetAffiliation("org4")
	assert.NoError(t, err, "Failed to rename 'org2' to 'org4' successfully")
