...
func TestIdentityCmd(t *testing.T) {
	...
	err = RunMain([]string{
		...
		"--getallids", "--getid",
		"testuser"})
	assert.Error(t, err, "Should have errored if requesting all ids and a specific id")

	err = RunMain([]string{
		cmdName, "identity"})
	assert.Error(t, err, "Should have errored if flags or subcommand specified")
	...
	err = RunMain([]string{
		cmdName, "identity", "modify", "--name", "testuser1", "--type", "peer"})
