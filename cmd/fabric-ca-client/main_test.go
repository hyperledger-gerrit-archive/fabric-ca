...
func checkCRL(t *testing.T, client *lib.Client, revokedSerials []*big.Int) {
	...
	assert.False(t, revokedList.HasExpired(time.Now().UTC().AddDate(0, 0, crlExpiry-1)), "Next Update value is not set to expected value (10 days)")
	assert.True(t, revokedList.HasExpired(time.Now().UTC().AddDate(0, 0, crlExpiry+1)), "Next Update value is not set to expected value (10 days)")
