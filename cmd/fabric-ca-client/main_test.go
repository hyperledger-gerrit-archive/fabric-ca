...
func testRevoke(t *testing.T) {
	...
	err = RunMain([]string{cmdName, "revoke", "-u", serverURL, "--revoke.name",
		"testRegister4", "--revoke.serial", "", "--revoke.aki", "", "--revoke.gencrl"})
	if err != nil {
	...
	assert.NoError(t, err, "CRL should be created when revoke is called without --gencrl parameter")
	...
	// Revoke testRegister5 without --revoke.gencrl option, so it does not create a CRL
	err = RunMain([]string{cmdName, "revoke", "-u", serverURL, "--revoke.name",
...
func setupGenCRLTest(t *testing.T, adminHome string) *lib.Server {
	...
	srv.CA.Config.CRL.CertExpirationWindow = time.Hour
