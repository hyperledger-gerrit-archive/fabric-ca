...
func runGetTCerts(cmd *cobra.Command) error {
	...
	if err != nil {
		fmt.Errorf("failed loading mgr: %s", err)
		return nil
	}
	publicKeySet, publicKeyError := generatePublicKeys(num)
	if publicKeyError != nil {
		fmt.Errorf("Error in generating public key : %s", publicKeyError)
	}
	duration, durartionParseerror := time.ParseDuration("10h")
	if durartionParseerror != nil {
		fmt.Errorf("Time parsing failed with error : %s", durartionParseerror)
		return nil
	}
	...
	ecert,err := tcert.LoadCert(id.GetMyCertFile())
	if err != nil {
		fmt.Errorf("Error from load ecert: %s", err)
		return nil
	}
	...
	if err != nil {
		fmt.Errorf("Error from GetBatch: %s", err)
		return nil
	}
	tcerts := resp.TCerts
	for i:=0; i<num; i++{
...
func generatePublicKeys(num int) ([][]byte, error) {
	...
	for i := 0; i < num; i++ {
		...
		if privKeyError != nil {
			fmt.Errorf("Private Key Generation failed with error : %s ", privKeyError)
			return nil, privKeyError
		}

		publicKeyraw, pemEncodingError = x509.MarshalPKIXPublicKey(privKey.Public())
		if pemEncodingError != nil {
			fmt.Errorf("Pem encoding public Key failed with error : %s", pemEncodingError)
		...
		if pemEncodingError != nil {
			fmt.Errorf("Pem encoding private Key failed with error : %s", pemEncodingError)
			return nil, pemEncodingError
		}
