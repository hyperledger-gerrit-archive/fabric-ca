...
type crlArgs struct {
	...
	RevokedAfter string `help:"Generate CRL with unexpired certificates that were revoked after this UTC timestamp (in RFC3339 format)"`
	// Genenerate CRL with all the certificates that were revoked before or at this timestamp
	RevokedBefore string `help:"Generate CRL with unexpired certificates that were revoked before this UTC timestamp (in RFC3339 format)"`
}
