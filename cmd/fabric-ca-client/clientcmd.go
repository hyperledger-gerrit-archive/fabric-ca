...
type affiliationsCfg struct {
	// Force if set true will remove the requested affiliation and any identities associated with that affiliation
	Force bool `help:"Enables automatically removing an identity when the identity's affiliation is removed"`
}
