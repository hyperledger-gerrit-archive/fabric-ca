...
func (c *ClientCmd) configInit() error {
	...
	if c.cfgFileName == "" {
		c.cfgFileName = filepath.Base(util.GetDefaultConfigFile(cmdName))
	}

	if c.homeDirectory == "" {
		c.homeDirectory = filepath.Dir(util.GetDefaultConfigFile(cmdName))
	}
	...
	log.Debugf("Home directory is: %s", c.homeDirectory)
