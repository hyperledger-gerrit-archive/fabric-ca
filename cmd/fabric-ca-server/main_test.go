...
func TestErrors(t *testing.T) {
	...
	errorCases := []TestData{
		...
		{[]string{cmdName, "start", "-c", startYaml, "-b", "user:aBcdef1!!", "ca.key"}, "Unrecognized arguments found"},
	}
...
func TestOneTimePass(t *testing.T) {
	...
	// Test with "-b" option
	bflag := fmt.Sprintf("admin:%s", password.Generate())
...
func TestValid(t *testing.T) {
	...
	bFlagInput := fmt.Sprintf("admin:%s", password.Generate())

	os.Setenv("CA_CFG_PATH", ".")
	validCases := []TestData{
		{[]string{cmdName, "init", "-b", bFlagInput}, ""},
...
func TestDefaultMultiCAs(t *testing.T) {
	...
	bFlagInput := fmt.Sprintf("user:%s", password.Generate())
