...
func enrollBootstrapUser(surl *string, configHome *string,
	...
	if err != nil {
		log.Printf("Failed to store the CA chain: %v", err)
	}
	return
}
...
func storeCAChain(config *lib.ClientConfig, si *lib.GetServerInfoResponse) error {
	if !util.FileExists(config.MSPDir) {
		return fmt.Errorf("Directory does not exist: %s", config.MSPDir)
	}
	caCertsDir := path.Join(config.MSPDir, "cacerts")
	...
	log.Printf("Stored CA certificate chain at %s", path)
	return nil
}
