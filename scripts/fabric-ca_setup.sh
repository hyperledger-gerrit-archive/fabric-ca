...
function usage() {
   ...
   echo "  -X)   set PROXY='true' - start haproxy for \$COP_INSTANCES of fabric-ca servers"
...
function buildCop(){
   cd $COP
   make fabric-ca || ErrorExit "buildCop failed"
}

function resetCop(){
   killAllCops
   rm -rf $DATADIR
   rm $TESTDATA/fabric-ca.db
   cd /tmp
   /usr/bin/dropdb 'fabric-ca' -U postgres -h localhost -w
   mysql --host=localhost --user=root --password=mysql -e 'DROP DATABASE IF EXISTS `fabric-ca`;'
}
...
function listCop(){
   ...
   case $DRIVER in 
      mysql)
         echo ""
         mysql --host=localhost --user=root --password=mysql -e 'show tables' `fabric-ca`
         echo "Users:" 
         mysql --host=localhost --user=root --password=mysql -e 'SELECT * FROM "users";' `fabric-ca`
      ;;
      postgres) 
         echo ""
         runPSQL '\l fabric-ca' | sed 's/^/   /;1s/^ *//;1s/$/:/'

         echo "Users:" 
         runPSQL 'SELECT * FROM "users";' '--dbname=fabric-ca' | sed 's/^/   /'
      ;;
   esac
}

function initCop() {
   test -f $COPEXEC || ErrorExit "fabric-ca executable not found (use -B to build)"
   cd $COP/bin
...
case $DRIVER in 
   postgres) DATASRC="dbname=fabric-ca host=127.0.0.1 port=$POSTGRES_PORT user=postgres password=postgres sslmode=disable" ;;
   sqlite3)   DATASRC="fabric-ca.db" ;;
   mysql)    DATASRC="root:mysql@tcp(localhost:$MYSQL_PORT)/fabric-ca?parseTime=true" ;;
esac
