...
# Reenroll should fail if client uses an Idemix credential to send a reenroll request
$FABRIC_CA_CLIENTEXEC reenroll --csr.cn testGenCSR -H $CA_CFG_PATH/$USERNAME -d
test $? -eq 1 || ErrorMsg "Reenroll should have failed, since identity has no x509 credentials"
...
$FABRIC_CA_CLIENTEXEC register -H $CA_CFG_PATH/$USERNAME --id.name user$i --id.secret user$i -d
test $? -eq 1 || ErrorMsg "Should failed to complete 'register' command, CRI has been updated. Caller needs to get new CRI"

# Get CRI command and then try register again, it should succeed

$SCRIPTDIR/fabric-ca_setup.sh -K

psql -d fabric_ca -c "DROP DATABASE fabric_ca"
