...
grep 'id'$'\t''256' $TESTDIR/text.txt
if [ $? != 0 ]; then
    ErrorMsg "Database column 'id' should have character limit of 256"
fi
...
mysql --host=localhost --user=root --password=mysql --database=$DBNAME -e "SELECT column_name, character_maximum_length FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'certificates';" > $TESTDIR/text.txt
grep 'id'$'\t''256' $TESTDIR/text.txt
if [ $? != 0 ]; then
    ErrorMsg "Database column 'id' should have character limit of 256"
fi
...
###### POSTGRES ######
# Create the database tables using the old schema
echo "Creating '$DBNAME' Postgres database and tables befor starting up server"
...
psql -d $DBNAME -c "SELECT column_name, character_maximum_length FROM information_schema.columns where table_name = 'certificates' AND column_name = 'id';" | grep "256"
if [ $? != 0 ]; then
    ErrorMsg "Database column 'id' should have character limit of 256"
fi
