...
function registerEnroll() {
   #roles="$(for i in {1..100000}; do printf "X";done)"
   roles="role$1"
   utype="type$1"
   ...
   $FABRIC_CA_CLIENTEXEC identity modify userType1 $URI -d \
     -H $TESTDIR/admin2 --type client 2>&1 |
        grepPrint "401 Unauthorized" ||
           ErrorMsg "admin2 should not be able to modify user of 'type1'"
...
function testDelegation () {
   enroll
   # admin may not delegate roles not in his hf.Registrar.Roles list
...
function removeAllUsers() {
  ...
  for u in $( $FABRIC_CA_CLIENTEXEC identity list $URI -H /tmp/ident_modify/admin/ |
               awk '{for (i=1;i<=NR;i++) {gsub(/,/,"");if ($i=="Name:" && $(i+1)!="admin") print $(i+1) }}'); do
     $FABRIC_CA_CLIENTEXEC identity remove $u $URI -d -H /tmp/ident_modify/admin/ || ErrorMsg "Failed to delete user $u"
  done
  numUsers=$($FABRIC_CA_CLIENTEXEC identity list $u $URI -H /tmp/ident_modify/admin/ | wc -l)
  ...
  $FABRIC_CA_CLIENTEXEC identity remove id $URI -H /tmp/ident_modify/admin/ 2>&1 |
     grepPrint 'Failed to get User' ||
        ErrorMsg "Should have failed, or wrong error code"
  # attempt delete self w/o force
  $FABRIC_CA_CLIENTEXEC identity remove admin $URI -H /tmp/ident_modify/admin/ 2>&1 |
      grepPrint "Need to use 'force'" ||
         ErrorMsg "Should have failed, or wrong error code" 
  # delete self
  $FABRIC_CA_CLIENTEXEC identity remove admin $URI --force -H /tmp/ident_modify/admin/ ||
