...
for driver in sqlite3 ; do
   ...
   for DEL in client peer validator auditor; do
      ...
      REGISTRAR="admin"
      enrollUser A_$DEL $DEL "[{\"name\":\"hf.Registrar.Roles\",\"value\":\"${DEL}\"},{\"name\":\"hf.Registrar.DelegateRole\", \"value\": \"${DEL}\"}]"
      ...
      for REG in client peer validator auditor; do
         ...
         for ENR in client peer validator auditor; do
            ...
            rc=$?
            if test "$REG" == "$ENR" -a $rc -ne 0; then ErrorMsg "register E_$DEL$REG$ENR failed"; read x
