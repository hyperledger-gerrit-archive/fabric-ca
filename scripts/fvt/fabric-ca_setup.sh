...
function resetFabricCa(){
   ...
   runPSQL "\connect  $DBNAME" 2>/dev/null && /usr/bin/dropdb "$DBNAME" -U postgres -h localhost -w --if-exists
}
...
function listFabricCa(){
   ...
   case $DRIVER in
      mysql)
         echo ""
         mysql --host=localhost --user=root --password=mysql -e 'show tables' $DBNAME
         echo "Users:"
