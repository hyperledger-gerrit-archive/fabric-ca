...
resetFabricCa(){
   ...
   for i in "" $(seq ${CACOUNT:-0}); do
      mysql --host=localhost --user=root --password=mysql -e 'show tables' $DBNAME$i >/dev/null 2>&1
         mysql --host=localhost --user=root --password=mysql -e "DROP DATABASE IF EXISTS $DBNAME$i" >/dev/null 2>&1
      /usr/bin/dropdb "$DBNAME$i" -U postgres -h localhost -w --if-exists 2>/dev/null
   done
}
...
listFabricCa(){
   ...
   for i in "" $(seq ${CACOUNT:-0}); do
      echo ""
      echo " ======================================"
      echo " ========> Dumping $DBNAME$i Database"
      echo " ======================================"
      case $DRIVER in
         mysql)
            echo ""
            echo "Users:"
            mysql --host=localhost --user=root --password=mysql -e 'SELECT * FROM users;' $DBNAME$i
            if $($FABRIC_CA_DEBUG); then
               echo "Certificates:"
               mysql --host=localhost --user=root --password=mysql -e 'SELECT * FROM certificates;' $DBNAME$i
               echo "Affiliations:"
               mysql --host=localhost --user=root --password=mysql -e 'SELECT * FROM affiliations;' $DBNAME$i
            fi
         ;;
         postgres)
            echo ""
            runPSQL "\l $DBNAME$i" | sed 's/^/   /;1s/^ *//;1s/$/:/'

            echo "Users:"
            runPSQL "SELECT * FROM USERS;" "--dbname=$DBNAME$i" | sed 's/^/   /'
            if $($FABRIC_CA_DEBUG); then
               echo "Certificates::"
               runPSQL "SELECT * FROM CERTIFICATES;" "--dbname=$DBNAME$i" | sed 's/^/   /'
               echo "Affiliations:"
               runPSQL "SELECT * FROM AFFILIATIONS;" "--dbname=$DBNAME$i" | sed 's/^/   /'
            fi
         ;;
...
#                     --ca.keyfile $DST_KEY $cacount --config $RUNCONFIG $args > $DATADIR/server${port}.log 2>&1 &
   ...
   pollServer "$FABRIC_CA_SERVEREXEC" "$polladdr" "$port" "$timeout" "start"
   if test "$?" -eq 0; then
      echo " STARTED"
   else
      RC=$((RC+1))
