...
function getCaCert() {
   ...
   i=0;while test $((i++)) -lt $NUMINTCAS; do
      ...
      caCertFile=$(echo ${ADDR}|sed 's/\./-/g')-7054.pem

      /usr/local/bin/fabric-ca-client getcacert -u ${PROTO}admin:adminpw@$ADDR:7054 #-c "$TDIR/int${i}/fabric-ca-client-config.yaml"
      # if the file didn't get created, fail
      test -f "$intDir/$caCertFile" || return 1
