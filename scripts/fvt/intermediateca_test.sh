...
# Start intermediate CAs
   ...
   i=0;while test $((i++)) -lt $NUMINTCAS; do
      ADDR=127.0.${i}.1
      pollServer fabric-ca-server $ADDR $CA_PORT $TIMEOUT
   done
}
...
function enrollUser() {
   ...
   i=0;while test $((i++)) -lt $NUMINTCAS; do
      ...
      /usr/local/bin/fabric-ca-client enroll \
                      ...
                      -u ${PROTO}admin:adminpw@$ADDR:${CA_PORT} \
...
function getCaCert() {
   ...
   i=0;while test $((i++)) -lt $NUMINTCAS; do
      ...
      caCertFile=$(echo ${ADDR}|sed 's/\./-/g')-${CA_PORT}.pem
...
function registerAndEnrollUser() {
   local rc=0
   i=0;while test $((i++)) -lt $NUMINTCAS; do
      pswd=$(/usr/local/bin/fabric-ca-client register -u ${PROTO}admin:adminpw@$ADDR:${CA_PORT} \
                              --id.name user${i} \
                              --id.type user \
                              --id.maxenrollments $MAXENROLL \
                              --id.affiliation org1 \
      ...
      /usr/local/bin/fabric-ca-client enroll \
                         --id.maxenrollments $MAXENROLL \
                         -u ${PROTO}user${i}:$pswd@$ADDR:${CA_PORT} \
