...
func (fr *flagRegistrar) shouldRegister(f *Field) bool {
	...
	// Don't register field if skip tag is present
	if fr.getTag(f, TagSkip) == "true" {
		return false
	}

	// Field with no component specified should not be registered
	if component == "" {
		return false
	}
	...
	for _, comp := range componentList {
		if comp == fr.component {
			// Server currently only has global level flags, if field has component tag to
			...
			// Look through the command tag to see which client commands can register this field/flag
