...
func CreateToken(csp bccsp.BCCSP, cert []byte, key bccsp.Key, method, path string, body []byte) (string, error) {
	...
	*/
	case *ecdsa.PublicKey:
		token, err = GenECDSAToken(csp, cert, key, method, path, body)
		if err != nil {
			return "", err
		}
	}
...
func GenECDSAToken(csp bccsp.BCCSP, cert []byte, key bccsp.Key, method, path string, body []byte) (string, error) {
	b64body := B64Encode(body)
	b64cert := B64Encode(cert)
	bodyAndcert := method + "." + path + "." + b64body + "." + b64cert
	log.Debugf("bodyAndcert=%s\n", bodyAndcert)
...
func VerifyToken(csp bccsp.BCCSP, token string, method, path string, body []byte) (*x509.Certificate, error) {
	...
	b64Body := B64Encode(body)
	sigString := method + "." + path + "." + b64Body + "." + b64Cert
	log.Debugf("sigString=%s\n", sigString)
