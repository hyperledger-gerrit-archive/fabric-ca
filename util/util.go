...
func VerifyToken(csp bccsp.BCCSP, token string, method, uri string, body []byte) (*x509.Certificate, error) {
	...
	newAuthHdr := os.Getenv("FABRIC_CA_SERVER_NEWAUTHHDR")
	newAuthHdr = "false" // TODO: Remove this default setting once all the SDKs have been updated to use the new authorization header
	...
	if strings.ToLower(newAuthHdr) != "true" && !valid {
