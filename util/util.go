...
func GenECDSAToken(csp bccsp.BCCSP, cert []byte, key bccsp.Key, body []byte) (string, error) {
	...
	if digestError != nil {
		return "", errors.Wrapf(digestError, "Hash failed on '%s'", bodyAndcert)
	}

	ecSignature, err := csp.Sign(key, digest, nil)
	if err != nil {
		return "", errors.Wrap(err, "BCCSP signature generation failure")
	}
...
func VerifyToken(csp bccsp.BCCSP, token string, body []byte) (*x509.Certificate, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "Invalid base64 encoded signature in token")
	}
	b64Body := B64Encode(body)
	...
	if err != nil {
		return nil, errors.Wrap(err, "Public Key import into BCCSP failed with error")
	}
	if pk2 == nil {
	...
	if digestError != nil {
		return nil, errors.Wrap(digestError, "Message digest failed")
	}

	valid, validErr := csp.Verify(pk2, sig, digest, nil)

	if validErr != nil {
		return nil, errors.Wrap(validErr, "Token signature validation failure")
	}
	if !valid {
...
func DecodeToken(token string) (*x509.Certificate, string, string, error) {
	...
	if err != nil {
		return nil, "", "", errors.Wrap(err, "Failed to decode base64 encoded x509 cert")
	}
	x509Cert, err := GetX509CertificateFromPEM(certDecoded)
	if err != nil {
		return nil, "", "", errors.Wrap(err, "Error in parsing x509 certificate given block bytes")
	}
