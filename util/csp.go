...
func ConfigureBCCSP(optsPtr **factory.FactoryOpts, mspDir, homeDir string) error {
	...
	if err != nil {
		return errors.Wrap(err, "Failed to make BCCSP files absolute")
	}
	...
	if err != nil {
		return errors.Wrap(err, "Failed to initialize BCCSP Factories")
	}
	*optsPtr = opts
	return nil
}
...
func GetBCCSP(opts *factory.FactoryOpts, homeDir string) (bccsp.BCCSP, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Failed to get BCCSP with opts: %+v", opts)
	}
	return csp, nil
}
...
func BccspBackedSigner(caFile, keyFile string, policy *config.Signing, csp bccsp.BCCSP) (signer.Signer, error) {
	...
	if err != nil {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "Could not find the private key in BCCSP keystore nor in keyfile '%s'", keyFile)
		}

		signer, err = cspsigner.New(csp, key)
		if err != nil {
			return nil, errors.Wrap(err, "Failed initializing CryptoSigner")
		}
		cspSigner = signer
	}
...
func GetSignerFromCert(cert *x509.Certificate, csp bccsp.BCCSP) (bccsp.Key, crypto.Signer, error) {
	...
	if err != nil {
		return nil, nil, errors.Wrap(err, "Failed to import certificate's public key")
	}
	// Get the key given the SKI value
	privateKey, err := csp.GetKey(certPubK.SKI())
	if err != nil {
		return nil, nil, errors.Wrap(err, "Could not find matching private key for SKI")
	}
	// Construct and initialize the signer
	signer, err := cspsigner.New(csp, privateKey)
	if err != nil {
		return nil, nil, errors.Wrap(err, "Failed to load ski from bccsp")
	}
...
func BCCSPKeyRequestGenerate(req *csr.CertificateRequest, myCSP bccsp.BCCSP) (bccsp.Key, crypto.Signer, error) {
	...
	if err != nil {
		return nil, nil, errors.Wrap(err, "Failed initializing CryptoSigner")
	}
...
func ImportBCCSPKeyFromPEM(keyFile string, myCSP bccsp.BCCSP, temporary bool) (bccsp.Key, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Failed parsing private key from %s", keyFile)
	}
	switch key.(type) {
	case *ecdsa.PrivateKey:
		priv, err := utils.PrivateKeyToDER(key.(*ecdsa.PrivateKey))
		if err != nil {
			return nil, errors.Wrapf(err, "Failed to convert ECDSA private key for '%s'", keyFile)
		}
		sk, err := myCSP.KeyImport(priv, &bccsp.ECDSAPrivateKeyImportOpts{Temporary: temporary})
		if err != nil {
			return nil, errors.Wrapf(err, "Failed to import ECDSA private key for '%s'", keyFile)
		}
		return sk, nil
	case *rsa.PrivateKey:
...
func LoadX509KeyPair(certFile, keyFile string, csp bccsp.BCCSP) (*tls.Certificate, error) {
	...
	if err != nil {
		if keyFile != "" {
			...
			return nil, errors.Wrap(err, "Could not load TLS certificate with BCCSP")
		}
