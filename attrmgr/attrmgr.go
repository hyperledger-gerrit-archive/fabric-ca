...
func (mgr *Mgr) ProcessAttributeRequests(reqs []AttributeRequest, attrs []Attribute) (string, string, error) {
	...
	if len(missingRequiredAttrs) > 0 {
		return "", "", errors.Errorf("The following required attributes are missing: %+v",
...
func (sai *SecretAttrInfo) add(name string, key []byte, idx int) error {
	if sai.Attrs[name] != nil {
		return errors.Errorf("Attribute '%s' is already set", name)
	}
...
func encryptString(str string, csp bccsp.BCCSP) ([]byte, []byte, error) {
	...
	/*
		...
		if err != nil {
			return nil, nil, errors.WithMessage(err, "Failed to generate key")
		}
		key, err := bkey.Bytes()
		if err != nil {
			return nil, nil, errors.WithMessage(err, "Failed to serialize key")
		}
		cipherText, err := csp.Encrypt(bkey, []byte(str), &bccsp.AESCBCPKCS7ModeOpts{})
		if err != nil {
			return nil, nil, errors.WithMessage(err, "Encryption failure")
		}
...
func decryptString(key []byte, cipherText []byte, csp bccsp.BCCSP) (string, error) {
	...
	/*
		...
		if err != nil {
			return "", errors.WithMessage(err, "Failed to import key")
		}
		// Decrypt using key
		val, err := csp.Decrypt(k, cipherText, bccsp.AESCBCPKCS7ModeOpts{})
		if err != nil {
			return "", errors.WithMessage(err, "Decryption failure")
		}
