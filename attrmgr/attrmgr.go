...
// New constructs a certificate manager
...
// Mgr is the certificate manager and is the main object for this package
type Mgr struct {
	csp bccsp.BCCSP
}
...
func (mgr *Mgr) ProcessAttributeRequestsForCert(reqs []AttributeRequest, attrs []Attribute, cert *x509.Certificate) ([]byte, error) {
...
// and returns a serialized version of PublicAttrInfo and SecretAttrInfo, respectively.
// The PublicAttrInfo goes inside the certificate and the SecretAttrInfo is separate from the certificate.
func (mgr *Mgr) ProcessAttributeRequests(reqs []AttributeRequest, attrs []Attribute) ([]byte, []byte, error) {
	var key, val []byte
	var err error
	...
	if len(missingRequiredAttrs) > 0 {
		return nil, nil, fmt.Errorf("The following required attributes are missing: %+v",
			missingRequiredAttrs)
	}
	return marshal(pai), marshal(sai), nil
}

// AddAttributesToCert adds public attribute info to a certificate.
func (mgr *Mgr) AddAttributesToCert(publicAttrInfo []byte, cert *x509.Certificate) {
	ext := pkix.Extension{
		Id:       AttrOID,
		Critical: false,
		Value:    publicAttrInfo,
	}
...
func (mgr *Mgr) GetAttributesFromCert(cert *x509.Certificate, secret []byte) (*Attributes, error) {
	var err error
	// We will read from 'pai' and 'sai' and populate 'attrs'
	pai := newPublicAttrInfo()
	sai := newSecretAttrInfo()
	attrs := map[string]*attrInfo{}
	// Populate 'pai' from the attribute info from the certificate
	cai := getAttrInfoFromCert(cert)
	if cai != nil {
		err = unmarshal("public attribute info", cai, pai)
		if err != nil {
			return nil, err
		}
	}
	// Populate 'sai' with attribute info in the 'secret' parameter
	if secret != nil {
		err = unmarshal("secret attribute info", secret, sai)
		if err != nil {
			return nil, err
		}
	}
	...
	// Get the certificate attribute info
...
// Names returns the names of the attributes that are in this certificate
...
// Contains returns true if the certificate contains the attribute
...
func getAttrInfoFromCert(cert *x509.Certificate) []byte {
...
// Marshal an arbitrary object and return its bytes
...
// Unmarshal bytes into an arbitrary structure
func unmarshal(name string, data []byte, obj interface{}) error {
...
func encryptString(str string, csp bccsp.BCCSP) ([]byte, []byte, error) {
	return []byte("key"), []byte(str), nil
	/* TODO: Fix; Bytes() is not supported
