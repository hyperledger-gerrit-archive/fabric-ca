...
fabric-ca is still being developed.
This section describes what you can currently do with fabric-ca.

### Prerequisites

* Go 1.6+ installation or later
* **GOPATH** environment variable is set correctly
* **FCA** environment variable is set to **$GOPATH/src/github.com/hyperledger/fabric-ca**

### Download and build the fabric-ca executable

The following shows how to download and build the fabric-ca executable (i.e. the 'fabric-ca' binary).
...
The executable is at `$FCA/bin/fabric-ca`.
...
seen below and placed in the client home directory. The **ca_certfiles** option is
...
See the `$FCA/testdata/server-config.json` file and note the "admin" user with a password of "adminpw".
...
The default value of the `COP_ENROLLMENT_DIR` environment variable is `$FABRIC_CA_HOME`.
...
See `$FCA/testdata/testconfig-ldap.json` for the complete configuration file with this section.  Also see `$FCA/scripts/run-ldap-tests` for a script which starts an OpenLDAP docker image, configures it, runs the LDAP tests in $FCA/cli/server/ldap/ldap_test.go, and stops the OpenLDAP server.
