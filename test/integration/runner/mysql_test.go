...
var _ = FDescribe("MySQL Runner", func() {
	...
	//	FContext("when the database uses TLS", func() {
	//		BeforeEach(func() {
	//			mysqlDB.HostIP = "127.0.0.1"
	//			port, err := runner.RandomPort()
	//			Expect(err).NotTo(HaveOccurred())
	//			mysqlDB.HostPort = port
	//			mysqlDB.TLSInfo.Enabled = true
	//			mysqlDB.TLSInfo.CACertFile = "/opt/gopath/src/github.com/hyperledger/fabric-ca/testdata/ca-cert.pem"
	//
	//			tlsClient := &runner.TLSClient{
	//				CertFile: "/opt/gopath/src/github.com/hyperledger/fabric-ca/testdata/tls_client-cert.pem",
	//				KeyFile:  "/opt/gopath/src/github.com/hyperledger/fabric-ca/testdata/tls_client-key.pem",
	//			}
	//			mysqlDB.TLSInfo.Client = tlsClient
	//		})
	//
	//		It("allows connections for execution", func() {
	//			err := mysqlDB.Start()
	//			Expect(err).NotTo(HaveOccurred())
	//		})
