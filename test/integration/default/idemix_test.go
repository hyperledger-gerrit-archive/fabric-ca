...
func TestIdemixRevoke(t *testing.T) {
	...
	err = command.RunMain([]string{cmdName, "revoke", "idemix", "--revocationhandle", crs[0].RevocationHandle, "-d"})
	...
	err = command.RunMain([]string{cmdName, "revoke", "idemix", "--name", "idemixUser", "-d"})
