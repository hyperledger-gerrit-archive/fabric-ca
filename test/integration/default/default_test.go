...
func TestMain(m *testing.M) {
	...
	if err != nil {
		clog.Errorf("Failed to get instance of server: %s", err)
		os.Exit(1)
	}

	err = defaultServer.Start()
	if err != nil {
		clog.Errorf("Failed to start server: %s", err)
		os.Exit(1)
	}
	...
	if err != nil {
		clog.Errorf("Failed to stop server: %s, integration test results: %d", err, rc)
		os.Exit(1)
	}
...
func TestListCertificateCmdPositive(t *testing.T) {
	...
	err = command.RunMain([]string{cmdName, "certificate", "list", "--id", "expire1", "--store", storeCertsDir})
	assert.NoError(t, err, "Should not error if the ID does not exist")
